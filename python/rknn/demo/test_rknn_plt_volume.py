...
RKNN_MODEL = '../rknn_model/PERSON_PLT_VOLUME.rknn'
device_id = '420035ef8cdff1f9'
#DATASET = './dataset.txt'
test_dir = "/media/y/GUTAI/windows/cell/datasets/volume/volume_from_wang_20231203/RBC"
save_dir = "./plt_volume"
nums_persample = 40
...
if __name__ == '__main__':
    ...
    for sample_file_name in os.listdir(test_dir):

        sample_dir = os.path.join(test_dir, sample_file_name)
        area_box = []
        count_img_nums = 0
        for filename in os.listdir(sample_dir):
            if count_img_nums > nums_persample:
                print(count_img_nums, nums_persample)
                continue
            count_img_nums += 1
            print(filename)
            ...
            for i in thr_thresholds:
                ...
                img_1 = cv2.cvtColor(img_ori, cv2.COLOR_RGB2BGR)
                if boxes is not None:
                    # draw(img_1, boxes, scores, classes)
                    # cv2.imwrite(save_path, img_1)
                    new_boxes = []
                    for box in boxes:
                        left, top,  right, bottom = recover_box(box)
                    ...
                    # draw(img_1, new_boxes, scores, classes)
                    # cv2.imwrite(save_path, img_1)
        ...
        print(mean_value)
        area_dict[sample_file_name]= mean_value
    print(area_dict)
    rknn.release()
