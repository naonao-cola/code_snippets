...
class DEIMWrapper:
    ...
    def allocate_memory(self, batch):
        ...
        # 输出形状根据你的 ONNX 模型
        self.output_labels = np.empty((self.batch_size, 300), dtype=np.int64)  # labels
        ...
        self.d_orig_target_sizes = cuda.mem_alloc(batch.shape[0] * 2 * np.int64().itemsize)  # orig_target_sizes
        ...
        assert len(tensor_names) == 5, "TensorRT 模型的输入+输出数量必须为5"
        ...
        self.context.set_tensor_address(tensor_names[2], int(self.d_output_labels))  # labels
        self.context.set_tensor_address(tensor_names[3], int(self.d_output_boxes))  # boxes
        self.context.set_tensor_address(tensor_names[4], int(self.d_output_scores))  # scores
    ...
    def preprocess(self, image):
        ...
        h, w, _  = image.shape
        orig_size = np.array([w, h], dtype=np.int64)[None] 
...
if __name__ == "__main__":
    trt_model = DEIMWrapper(file="models_tensorRT/DEIM.engine")
