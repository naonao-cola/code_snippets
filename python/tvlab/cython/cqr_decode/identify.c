...
static int read_cell(const struct quirc *q, int index, int x, int y)
{
    ...
    score = fitness_cell(q, index, x, y);
    if (score >= 7)
        return 1;
    if (score <= -3)
        return -1;
	...
	for (i = 0; i < 2; i++) {
        int s;
		s = fitness_cell(q, index, x - 1 + i, y - 1);
        ring_score += s >= 3 ? 1 : -1;
		s = fitness_cell(q, index, x - 1, y + 1 - i);
        ring_score += s >= 3 ? 1 : -1;
		s = fitness_cell(q, index, x + 1, y - 1 + i);
        ring_score += s >= 3 ? 1 : -1;
		s = fitness_cell(q, index, x + 1 - i, y + 1);
...
static int fitness_all_cnt(const struct quirc *q, int index)
{
	const struct quirc_grid *qr = &q->grids[index];
	int version = (qr->grid_size - 17) / 4;
	const struct quirc_version_info *info = &quirc_version_db[version];
...
static int jiggle_perspective(struct quirc *q, int index, struct quirc_point* rect, int size)
{
	struct quirc_grid *qr = &q->grids[index];
    int *xy_arr = (int*)rect;
    ...
    for (pass = 0; pass < 4; pass++) {
        for (i = 0; i < 16; i++) {
            ...
            xy_arr[j] = new;
            perspective_setup(qr->c, rect, size, size);
...
static void setup_qr_perspective(struct quirc *q, int index)
{
	...
	best_score = jiggle_perspective(q, index, rect, qr->grid_size-7);
    score_cnt = fitness_all_cnt(q, index);
    qr->score = (best_score * 100) / score_cnt;
}
...
void quirc_end_skip_detect(struct quirc *q, int32_t *qr_infos_ptr, int cnt) {
	uint8_t threshold = otsu(q);
	pixels_setup(q, threshold);
    ...
    for (int index = 0; index < cnt; index++) {
        struct quirc_grid *qr = &q->grids[index];
        struct quirc_point rect[4];
        int32_t* info_ptr = qr_infos_ptr + index*(9*sizeof(int32_t));
        int *xy_arr = (int*)rect;
        int best_score, score_cnt;
        qr->grid_size = info_ptr[8];

        for (int i = 0; i < 8; i++)
            xy_arr[i] = info_ptr[i];
        perspective_setup(qr->c, rect, qr->grid_size, qr->grid_size);
        best_score = jiggle_perspective(q, index, rect, qr->grid_size);
        score_cnt = fitness_all_cnt(q, index);
        qr->score = (best_score * 100) / score_cnt;
    }
    q->num_grids = cnt;
