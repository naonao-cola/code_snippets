...
static quirc_decode_error_t correct_block(uint8_t *data,
					  const struct quirc_rs_params *ecc, int *err_cnt)
{
...
static quirc_decode_error_t codestream_ecc(struct quirc_data *data,
					   struct datastream *ds, int *err_cnt)
{
    ...
    *err_cnt = 0;
	...
	for (i = 0; i < bc; i++) {
		...
		err = correct_block(dst, ecc, err_cnt);
		if (err)
			return err;
				  ...
				  struct quirc_data *data)
{
	...
	err = codestream_ecc(data, &ds, &err_cnt);
	if (err)
		return err;
    data->ecc_rate = (100*err_cnt)/ds.data_bits;
