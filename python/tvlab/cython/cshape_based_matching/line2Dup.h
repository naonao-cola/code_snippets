...
public:
    ColorGradientPyramid(const cv::Mat &src, const cv::Mat &mask,
                        ...
                        float strong_threshold, int gaussion_kernel_size);
...
public:
    ...
    {
        return cv::makePtr<ColorGradientPyramid>(src, mask, weak_threshold, num_features,
                strong_threshold, gaussion_kernel_size);
    }
};
...
public:
    ...
    Detector(int num_features, std::vector<int> T, float weak_thresh = 30.0f,
            float strong_thresh = 60.0f, int gaussion_kernel_size=7);
    ...
    int addTemplate(const cv::Mat sources, const std::string &class_id,
                    ...
                    int real_width=128, int real_height=128,
    ...
    int hasTemplates(const std::string &class_id, int template_id);
...
public:
    ...
    void point_of(const Info& info, int x, int y, int &dst_x, int &dst_y){
        cv::Mat dst;

        cv::Point2f center(src.cols/2.0f, src.rows/2.0f);
        cv::Mat_<float> rot_mat = cv::getRotationMatrix2D(center, -info.angle, info.scale);

        cv::Point2f p(x,y);
        cv::Mat_<float> pm(3,1);
        pm << p.x,p.y,1.0;

        cv::Mat_<float> pr =  rot_mat * pm;
