...
int dongle_write_file(int index, unsigned char *file_data, int size)
{
    ...
    unsigned long wOffset = 10;	//从偏移位置为10的地方开始写入
    int dwRet = 0;

    cBuffer[0] = 0;								//为0，表示写文件
    cBuffer[1] = (unsigned char)size;					//写入数据长度
    FlipBuffer((unsigned char*)&wOffset, 2);	//C51中WORD与PC中顺序不同，先倒序
    memcpy(cBuffer+2, &wOffset, 2);				//将cBuffer中的下标为2，3中设置为偏移量
    memcpy(cBuffer+4, file_data, size); //要写入的数据
...
int dongle_read_file(int index, unsigned char *file_data, int size)
{
    ...
    cBuffer[0] = 1;								//为1，表示读文件
    cBuffer[1] = (unsigned char)size;					//读取数据长度
    FlipBuffer((unsigned char*)&wOffset, 2);	//C51中WORD与PC中顺序不同，先倒序
    memcpy(cBuffer+2, &wOffset, 2);				//将cBuffer中的下标为2，3中设置为偏移量
