...
class det_rec_functions(object):
    ...
    def __init__(self, image, use_large=False):
        self.img = image.copy()
        self.det_file = r'F:\new_file\demo\cxx\lego\models\ort_models\ch_PP-OCRv4_det_infer\reshape\det.onnx'
        ...
        self.postprocess_op = process_pred(
            r'F:\demo\3rdparty\PaddleOCR_TensorRT_cpp-main\data\ppocr_keys_v1.txt', 'ch', True)
    ...
    def recognition_img(self, dt_boxes):
        ...
        results_info = []
        for pic in img_list:
            res = self.get_img_res(self.onet_rec_session,
                                   pic, self.postprocess_op)
            results.append(res[0])
            results_info.append(res)
        return results, results_info

    def recognition_small_img(self, small_img):
        img_ori = self.img
        img = img_ori.copy()
        # 识别过程
        img_list = []
...
if __name__ == '__main__':
    # 读取图片
    image = cv2.imread(
        r'F:\new_file\EvopactHVX-210\Panel\111.jpg')
    # OCR-检测-识别
    ...
    results, results_info = ocr_sys.recognition_small_img(image)
    print(results)
