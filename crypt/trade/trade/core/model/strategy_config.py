...
class StrategyConfig:
    ...
    # 多头因子名称
    long_factor: str = '多头因子'
    
    # 空头因子名称
    short_factor: str = '空头因子'
    ...
    # 交易市场类型，格式为"多头市场_空头市场"，可选值为"spot"(现货)或"swap"(合约)
    # 例如："swap_swap"表示多空都使用合约，"spot_swap"表示多头使用现货空头使用合约，"spot_spot"表示多空都使用现货
    # 注意：当多头或空头使用现货时，对应方向无法做空
    ...
    # 多头选币因子列表，用于多头策略的因子配置
    long_factor_list: List[FactorConfig] = ()  # 因子名（和factors文件中相同），排序方式，参数，权重。
    
    # 空头选币因子列表，用于空头策略的因子配置
    short_factor_list: List[FactorConfig] = ()  # 因子名（和factors文件中相同），排序方式，参数，权重。

    # 全局过滤因子列表，应用于多空选币前的整体过滤
    filter_list: List[FilterFactorConfig] = ()  # 因子名（和factors文件中相同），参数
    
    # 多头前置过滤因子列表，仅应用于多头选币前
    long_filter_list: List[FilterFactorConfig] = ()  # 因子名（和factors文件中相同），参数
    
    # 空头前置过滤因子列表，仅应用于空头选币前
    short_filter_list: List[FilterFactorConfig] = ()  # 因子名（和factors文件中相同），参数
    
    # 多头后置过滤因子列表，应用于多头选币后
    long_filter_list_post: List[FilterFactorConfig] = ()  # 因子名（和factors文件中相同），参数
    
    # 空头后置过滤因子列表，应用于空头选币后
    short_filter_list_post: List[FilterFactorConfig] = ()  # 因子名（和factors文件中相同），参数
    ...
    @cached_property
    def long_is_spot(self):
        """多头是否使用现货"""
        return self.market.split('_')[0] == 'spot'
    
    @cached_property
    def short_is_spot(self):
        """空头是否使用现货"""
        return self.market.split('_')[1] == 'spot'
    
    @cached_property
    def is_pure_long(self):
        """是否为纯多模式
        如果多头使用现货且空头使用现货，则无法做空，即为纯多模式
        如果多头使用合约且空头使用现货，则也无法做空，即为纯多模式
        """
        return self.long_is_spot or self.short_is_spot
    ...
    def factor_columns(self) -> List[str]:
        ...
        # 针对多头因子信息
        for factor_config in self.long_factor_list:
            factor_columns.add(factor_config.col_name)
        
        # 针对空头因子信息
        for factor_config in self.short_factor_list:
            factor_columns.add(factor_config.col_name)
        ...
        # 针对当前策略的多头前置过滤因子信息，整理之后的列名信息，并且缓存到全局
        for filter_factor in self.long_filter_list:
            # 策略过滤因子最终在df中的列名
            factor_columns.add(filter_factor.col_name)  # 添加到当前策略缓存信息中
            
        # 针对当前策略的空头前置过滤因子信息，整理之后的列名信息，并且缓存到全局
        for filter_factor in self.short_filter_list:
            # 策略过滤因子最终在df中的列名
            factor_columns.add(filter_factor.col_name)  # 添加到当前策略缓存信息中
            
        # 针对当前策略的多头后置过滤因子信息，整理之后的列名信息，并且缓存到全局
        for filter_factor in self.long_filter_list_post:
            # 策略过滤因子最终在df中的列名
            factor_columns.add(filter_factor.col_name)  # 添加到当前策略缓存信息中
            
        # 针对当前策略的空头后置过滤因子信息，整理之后的列名信息，并且缓存到全局
        for filter_factor in self.short_filter_list_post:
            # 策略过滤因子最终在df中的列名
            factor_columns.add(filter_factor.col_name)  # 添加到当前策略缓存信息中
    ...
    def all_factors(self) -> set:
        # 合并所有因子和过滤因子
        all_factors = set(self.factor_list)
        all_factors.update(self.long_factor_list)
        all_factors.update(self.short_factor_list)
        all_factors.update(self.filter_list)
        all_factors.update(self.long_filter_list)
        all_factors.update(self.short_filter_list)
        all_factors.update(self.long_filter_list_post)
        all_factors.update(self.short_filter_list_post)
        return all_factors
    ...
    def init(cls, **config):
        ...
        # 初始化多头策略因子
        config['long_factor_list'] = FactorConfig.parse_config_list(config.get('long_factor_list', []))
        
        # 初始化空头策略因子
        config['short_factor_list'] = FactorConfig.parse_config_list(config.get('short_factor_list', []))
        ...
        # 初始化多头前置过滤因子
        config['long_filter_list'] = [FilterFactorConfig.init(filter_config) for filter_config in
                                     config.get('long_filter_list', [])]
        
        # 初始化空头前置过滤因子
        config['short_filter_list'] = [FilterFactorConfig.init(filter_config) for filter_config in
                                      config.get('short_filter_list', [])]
        
        # 初始化多头后置过滤因子
        config['long_filter_list_post'] = [FilterFactorConfig.init(filter_config) for filter_config in
                                         config.get('long_filter_list_post', [])]
        
        # 初始化空头后置过滤因子
        config['short_filter_list_post'] = [FilterFactorConfig.init(filter_config) for filter_config in
                                          config.get('short_filter_list_post', [])]

        # 开始初始化策略对象
        return cls(**config)

    def __repr__(self):
        return f"""- 持仓周期: {self.hold_period}
- 交易市场类型: {self.market}（格式：多头市场_空头市场，spot=现货，swap=合约）
- 多头是否使用现货: {bool_str(self.long_is_spot)}
- 空头是否使用现货: {bool_str(self.short_is_spot)}
- 是否纯多模式: {bool_str(self.is_pure_long)}
- 多头选币数量: {self.long_select_coin_num}
- 空头选币数量: {self.short_select_coin_num}
- 策略因子: {self.factor_list}
- 多头因子: {self.long_factor_list}
- 空头因子: {self.short_factor_list}
- 全局过滤因子: {self.filter_list}
- 多头前置过滤因子: {self.long_filter_list}
- 空头前置过滤因子: {self.short_filter_list}
- 多头后置过滤因子: {self.long_filter_list_post}
- 空头后置过滤因子: {self.short_filter_list_post}
- Offset列表: {self.offset_list}"""
    ...
    def calc_select_factor(self, df) -> pd.DataFrame:
        ...
        # 如果指定了多头因子列表，使用多头因子
        if self.long_factor_list:
            new_cols[self.long_factor] = calc_factor_common(df, self.long_factor_list)
        
        # 如果指定了空头因子列表，使用空头因子
        if self.short_factor_list:
            new_cols[self.short_factor] = calc_factor_common(df, self.short_factor_list)
        
        # 如果没有指定多空分离因子，则使用通用因子
        if not new_cols and self.factor_list:
            new_cols[self.factor_name] = calc_factor_common(df, self.factor_list)
            new_cols[self.long_factor] = new_cols[self.factor_name].copy()
            new_cols[self.short_factor] = new_cols[self.factor_name].copy()
    ...
    def filter_before_select(self, df):
        # 首先应用全局过滤因子
        global_filter_condition = filter_common(df, self.filter_list)
        df_global_filtered = df[global_filter_condition].copy()
        
        # 然后分别应用多头和空头的前置过滤因子
        long_filter_condition = filter_common(df_global_filtered, self.long_filter_list)
        short_filter_condition = filter_common(df_global_filtered, self.short_filter_list)
        
        return df_global_filtered[long_filter_condition].copy(), df_global_filtered[short_filter_condition].copy()
        
    def filter_after_select(self, long_df, short_df):
        # 应用多头和空头的后置过滤因子
        long_filter_condition = filter_common(long_df, self.long_filter_list_post)
        short_filter_condition = filter_common(short_df, self.short_filter_list_post)
        
        return long_df[long_filter_condition].copy(), short_df[short_filter_condition].copy()
