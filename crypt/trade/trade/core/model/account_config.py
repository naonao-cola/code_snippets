...
class AccountConfig:
    def __init__(self, name: str, **config):
        ...
        self.strategy_long: Optional[StrategyConfig] = None
        ...
        # 时间偏移量（分钟）
        self.time_offset_minutes: int = config.get("time_offset_minutes", 0)
    ...
    def is_pure_long(self):
        """是否为纯多模式
        从strategy.market派生，保持向后兼容
        - market格式为"多头市场_空头市场"
        - 当多头或空头使用现货时，对应方向无法做空，即为纯多模式
        """
        if not self.strategy:
            return True  # 默认返回纯多模式以确保安全
        return self.strategy.is_pure_long
...
"""
    ...
    @classmethod
    def init_from_account_dict(cls, account_dict: dict) -> 'AccountConfig':
        """
        从账户配置字典初始化AccountConfig
        """
        from config import exchange_basic_config
        # 创建一个副本，避免修改原始字典
        config_copy = account_dict.copy()
        name = config_copy.pop('name')  # 移除name，避免重复传递
        cfg = cls(name, **config_copy)
        cfg.load_strategy_config(account_dict['strategy'])
        cfg.init_exchange(exchange_basic_config)
        return cfg
    ...
    def load_strategy_config(self, strategy_dict: dict):
        ...
        # 初始化strategy_short和strategy_long属性
        self.strategy_short = strategy
        self.strategy_long = strategy
        
        # 初始化post_short和post_long属性
        self.post_short = {}
        self.post_long = {}
        
        # 为了保持向后兼容性，is_pure_long属性通过property从strategy.market派生
        # 注意：strategy对象会在后续代码中重新赋值，但这不影响property方法的正确工作
  
        if self.is_pure_long and self.leverage >= 2:
            print(f'⚠️ 纯多模式下，杠杆率不应大于1，当前杠杆为{self.leverage}')
            self.leverage = 1
        print(f'- 纯多模式: {bool_str(self.is_pure_long)}，当前杠杆率为{self.leverage}')
        ...
        # 检查strategy中是否已配置offset_list
        if hasattr(strategy, 'offset_list') and strategy.offset_list:
            print(f'- 使用strategy中配置的offset_list: {strategy.offset_list}')
        else:
            # 当未配置offset_list时，默认使用offset=0
            strategy.offset_list = (0,)
            print(f'- 未配置offset_list，默认使用offset=0: {strategy.offset_list}')
            
        # 兼容旧版本的use_offset参数（已废弃）
        if hasattr(self, 'use_offset') and self.use_offset:
            print(f'- 警告：use_offset参数已废弃，已改用strategy中的offset_list参数')
    ...
    def calc_order_amount(self, select_coin) -> pd.DataFrame:
        ...
        if select_coin.empty:
            # 如果选币结果为空，只使用当前持仓数据
            order_df = current_position[['symbol', 'symbol_type']].copy()
        else:
            # 如果选币结果不为空，合并选币结果和当前持仓
            order_df = pd.concat([
...
def load_accounts_config() -> list[AccountConfig]:
    """
    加载多账户配置
    """
    ...
    accounts = []
    for account_dict in accounts_config:
        if account_dict.get('enabled', True):  # 只加载启用的账户
            try:
                account = AccountConfig.init_from_account_dict(account_dict)
                accounts.append(account)
            except Exception as e:
                print(f"❌加载账户 {account_dict.get('name', '未知')} 失败: {e}")
                continue
...
def validate_accounts_config() -> tuple[bool, list[str]]:
    ...
    if not accounts_config:
        errors.append("❌未配置任何账户")
        return False, errors
    
    enabled_accounts = [acc for acc in accounts_config if acc.get('enabled', True)]
    if not enabled_accounts:
        errors.append("❌没有启用的账户")
        return False, errors
    
    account_names = set()
    time_offsets = set()
    
    for i, account_dict in enumerate(enabled_accounts):
        account_name = account_dict.get('name', f'账户{i+1}')
        
        # 检查必要字段
        if not account_dict.get('name'):
            errors.append(f"❌账户{i+1}: 缺少账户名称")
            continue
            
        if not account_dict.get('apiKey') or not account_dict.get('secret'):
            errors.append(f"❌{account_name}: 缺少API配置")
            continue
            
        if not account_dict.get('strategy'):
            errors.append(f"❌{account_name}: 缺少策略配置")
            continue
        
        # 检查账户名称唯一性
        if account_name in account_names:
            errors.append(f"❌{account_name}: 账户名称重复")
        else:
            account_names.add(account_name)
        
        # 检查时间偏移量
        time_offset = account_dict.get('time_offset_minutes', 0)
        if time_offset in time_offsets:
            errors.append(f"⚠️{account_name}: 时间偏移量 {time_offset} 分钟与其他账户冲突")
        else:
            time_offsets.add(time_offset)
        
        # 验证策略配置
        strategy = account_dict.get('strategy', {})
        # 支持传统的factor_list或者多空分离的long_factor_list和short_factor_list
        has_factor_list = bool(strategy.get('factor_list'))
        has_multi_factor_lists = bool(strategy.get('long_factor_list')) and bool(strategy.get('short_factor_list'))
        if not has_factor_list and not has_multi_factor_lists:
            errors.append(f"❌{account_name}: 策略缺少因子配置，请配置factor_list或同时配置long_factor_list和short_factor_list")
        
        valid_count += 1
    
    print(f"📊账户配置验证完成: {valid_count}/{len(enabled_accounts)} 个账户有效")
    
    return len(errors) == 0, errors


def print_accounts_summary(accounts: list[AccountConfig]):
    """
    打印所有账户的配置摘要
    """
    print("=" * 80)
    print(f"📋 多账户配置摘要 (共 {len(accounts)} 个账户)")
    print("=" * 80)
    
    for i, account in enumerate(accounts, 1):
        print(f"\n🏦 账户 {i}: {account.name}")
        print(f"   ├─ API配置: {'✅已配置' if account.is_api_ok() else '❌未配置'}")
        print(f"   ├─ 交易模式: {'🟢纯多' if account.is_pure_long else '🔄多空'}")
        print(f"   ├─ 持仓周期: {account.strategy.hold_period if account.strategy else '未知'}")
        print(f"   ├─ 选币数量: 多头{account.strategy.long_select_coin_num if account.strategy else '?'} | 空头{account.strategy.short_select_coin_num if account.strategy else '?'}")
        print(f"   ├─ 杠杆倍数: {account.leverage}x")
        print(f"   └─ 微信推送: {'✅已配置' if account.wechat_webhook_url else '❌未配置'}")
