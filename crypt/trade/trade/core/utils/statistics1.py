...
def save_and_send_equity_info(account_config: AccountConfig, swap_position, spot_equity, account_equity):
    ...
    # =使用实时API数据计算多头和空头仓位
    swap_account = account_config.bn.get_swap_account()
    positions = swap_account.get('positions', [])
    
    # 计算实时多头和空头仓位
    long_position_value = 0  # 多头合约价值
    short_position_value = 0  # 空头仓位价值
    
    for position in positions:
        position_amt = float(position.get('positionAmt', 0))
        if position_amt != 0:  # 只计算有持仓的合约
            mark_price = float(position.get('markPrice', 0))
            entry_price = float(position.get('entryPrice', 0))
            # 优先使用标记价格，如果为0则使用入场价格
            price = mark_price if mark_price > 0 else entry_price
            if price > 0:
                position_value = abs(position_amt) * price
                if position_amt > 0:  # 多头仓位
                    long_position_value += position_value
                else:  # 空头仓位
                    short_position_value += position_value
    
    # 记录到DataFrame中
    new_equity_df.loc[0, '多头合约'] = round(long_position_value, 2)  # 记录多头合约
    new_equity_df.loc[0, '多头仓位'] = round(spot_equity + long_position_value, 2)  # 记录多头仓位（现货+合约）
    new_equity_df.loc[0, '空头仓位'] = round(short_position_value, 2)  # 记录空头仓位
    
    # =追加信息到本地存储中（保留原有逻辑用于图表显示）
    ...
    # 获取账户概览信息用于详细显示
    account_overview = account_config.bn.get_account_overview()
    swap_account = account_config.bn.get_swap_account()
    
    # 获取合约账户详细信息
    total_wallet_balance = float(swap_account.get('totalWalletBalance', 0))  # 钱包余额
    total_margin_balance = float(swap_account.get('totalMarginBalance', 0))  # 账户总权益
    total_unrealized_pnl = float(swap_account.get('totalUnrealizedProfit', 0))  # 未实现盈亏
    available_balance = float(swap_account.get('availableBalance', 0))  # 可用保证金
    ...
    # 计算过去1小时盈亏
    if not old_equity_df.empty and len(old_equity_df) > 0:
        # 获取1小时前的净值数据
        one_hour_ago = datetime.now() - pd.Timedelta(hours=1)
        hour_ago_data = old_equity_df[old_equity_df['time'] <= one_hour_ago]
        if not hour_ago_data.empty:
            hour_ago_equity = hour_ago_data.iloc[-1]['账户总净值']  # 取最接近1小时前的数据
            current_equity = new_equity_df.loc[0, '账户总净值']
            hourly_pnl = round(current_equity - hour_ago_equity, 2)
        else:
            hourly_pnl = 0.0  # 如果没有1小时前的数据，设为0
    else:
        hourly_pnl = 0.0  # 如果没有历史数据，设为0
    
    # 构建新格式的推送消息
    equity_msg = f'👤 【{account_config.name}】\n\n'
    
    # 策略配置信息
    equity_msg += f'⚙ 【策略配置】\n'
    equity_msg += f'🔄 策略周期：{account_config.strategy.hold_period if account_config.strategy else "未知"}\n\n'
    
    # 策略因子统计
    if account_config.strategy and account_config.strategy.factor_list:
        equity_msg += f'📊 【策略因子统计】\n'
        for i, factor in enumerate(account_config.strategy.factor_list, 1):
            direction_icon = "📈" if factor.is_sort_asc else "📉"
            equity_msg += f'{direction_icon} 因子{i}：{factor.name}({factor.param}) 权重{factor.weight*100:.1f}%\n'
        equity_msg += '\n'
    
    # 过滤因子统计
    if account_config.strategy and account_config.strategy.filter_list:
        equity_msg += f'🔍 【过滤因子统计】\n'
        for i, filter_factor in enumerate(account_config.strategy.filter_list, 1):
            direction_icon = "📈" if filter_factor.is_sort_asc else "📉"
            method_str = f"{filter_factor.method.how}{filter_factor.method.range}" if filter_factor.method else "未知"
            equity_msg += f'{direction_icon} 过滤{i}：{filter_factor.name}({filter_factor.param}) {method_str}\n'
    equity_msg += f'⚖ 当前杠杆：{account_config.leverage:.2f}x\n\n'
    
    # 合约账户详情
    equity_msg += f'💼 【合约账户详情】\n'
    equity_msg += f'💰 账户总权益：{total_margin_balance:.2f}\n'
    equity_msg += f'💳 钱包余额：{total_wallet_balance:.2f}\n'
    equity_msg += f'📈 未实现盈亏：{total_unrealized_pnl:.2f}\n'
    equity_msg += f'⏰ 过去1小时盈亏：{hourly_pnl:.2f}\n\n'
    
    # 历史净值
    equity_msg += f'📈 【历史净值】\n'
    equity_msg += f'🔝 历史最高：{max_all_equity if not pd.isna(max_all_equity) else "--"}\n'
    equity_msg += f'🔻 历史最低：{min_all_equity if not pd.isna(min_all_equity) else "--"}\n\n'
    
    # 持仓详情
    equity_msg += f'📊 【持仓详情】\n'
    equity_msg += f'👛 仓位估值：{total_position_value:.2f}\n'
    equity_msg += f'📊 多头仓位：{round(spot_equity + long_position_value, 2):.2f}\n'
    equity_msg += f'📊 空头仓位：{round(short_position_value, 2):.2f}\n'
    
    # 显示具体持仓（如果有的话）
    if not swap_position.empty:
        # 获取有持仓的币种
        try:
            position_symbols = swap_position[swap_position['当前持仓量'] != 0]
            if not position_symbols.empty:
                for _, row in position_symbols.head(5).iterrows():  # 最多显示5个持仓
                    try:
                        symbol = row.name if hasattr(row, 'name') else row.get('symbol', 'UNKNOWN')
                        pnl = float(row.get('持仓盈亏', 0))
                        position_value = float(row.get('仓位价值', 0))
                        pnl_pct = (pnl / abs(position_value)) * 100 if position_value != 0 else 0
                        icon = "🟢" if pnl >= 0 else "🔴"
                        pnl_icon = "📈" if pnl >= 0 else "📉"
                        equity_msg += f'{icon} {symbol}: {pnl_icon}{pnl:.2f} ({pnl_pct:.2f}%)\n'
