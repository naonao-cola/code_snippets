...
from core.model.account_config import AccountConfig, StrategyConfig, load_config
...
def select_coins(account: AccountConfig):
    ...
    **实盘专属操作** - 在实盘过程中裁切最后一个周期的数据
    ...
    """
    s = time.time()
    print(f'- 过滤前币种数量：{len(factor_df)}')
    long_df, short_df = strategy.filter_before_select(factor_df)
    print(f'- 过滤后多头币种数量：{len(long_df)}，空头币种数量：{len(short_df)}')
    if strategy.is_pure_long:  # 使用现货数据，则在现货中进行过滤，并选币
        short_df = pd.DataFrame(columns=short_df.columns)
        print(f'- 纯多模式，空头币种数量重置为：{len(short_df)}')
    ...
    if not factor_df.empty:
        s = time.time()
        long_df = factor_df[factor_df['方向'] == 1].copy()
        short_df = factor_df[factor_df['方向'] == -1].copy()
        long_df, short_df = strategy.filter_after_select(long_df, short_df)
        factor_df = pd.concat([long_df, short_df], ignore_index=True)
        if strategy.is_pure_long:
            short_df = pd.DataFrame(columns=short_df.columns)
        print(f'- 后置过滤后多头币种数量：{len(long_df)}，空头币种数量：{len(short_df)}')
        print(f'- 后置过滤耗时：{time.time() - s:.2f}s')

    result_df = factor_df[[*FACTOR_KLINE_COL_LIST, '方向', 'target_alloc_ratio']].copy()
    print(f'- 选币后结果数量：{len(result_df)}')

    if result_df.empty:
        print('⚠️ 选币结果为空，可能原因：1)过滤条件过严 2)选币数量为0 3)没有符合条件的币种')
        return pd.DataFrame()
    ...
    if result_df.empty:
        print('⚠️ offset筛选后结果为空，当前时间可能不在配置的offset范围内')
        return pd.DataFrame()
    ...
    # 当包含多个时间周期的数据时，只保留最新时间的选币结果
    if not select_result_df.empty:
        latest_time = select_result_df['candle_begin_time'].max()
        select_result_df = select_result_df[select_result_df['candle_begin_time'] == latest_time].copy()
        print(f'- 保留最新时间 {latest_time} 的选币结果，最终数量：{len(select_result_df)}')
...
def select_long_and_short_coin(strategy: StrategyConfig, long_df: pd.DataFrame, short_df: pd.DataFrame):
    ...
    # 做空选币
    """
    if not strategy.is_pure_long:  # 非纯多模式下，要计算空头选币
