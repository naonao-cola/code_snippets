...
from config import backtest_path, use_offset, is_pure_long, backtest_name
...
"""
    ...
    def load_strategy_config(self, strategy_dict: dict):
        ...
        self.is_use_spot = strategy_cfg.is_use_spot = is_pure_long  # 纯多的时候默认使用现货，多空的时候使用纯合约
        if is_pure_long:
            strategy_cfg.long_cap_weight = 1
            strategy_cfg.short_cap_weight = 0
        ...
        if strategy_cfg.long_select_coin_num == 0 and (strategy_cfg.short_select_coin_num == 0 or
            ...
            exit(1)
        elif strategy_cfg.short_select_coin_num == 0 and strategy_cfg.long_cap_weight == 0:
            print('❌ 策略中启用了全空模式，但是选币数量是0，请检查config.py')
            exit(1)

        # 根据配置更新offset的覆盖
        if use_offset:
            strategy_cfg.offset_list = list(range(0, strategy_cfg.period_num, 1))
