...
def select_coins(conf: BacktestConfig):
    ...
    ** 策略选币 **
    ...
    print(f'[{strategy.name}] 选币数据准备完成，消耗时间：{time.time() - s:.2f}s')
    ...
    print(f'[{strategy.name}] 选币因子计算耗时：{time.time() - s:.2f}s')
    ...
    if is_pure_long:  # 使用现货数据，则在现货中进行过滤，并选币
        short_df = pd.DataFrame(columns=short_df.columns)
    print(f'[{strategy.name}] 过滤耗时：{time.time() - s:.2f}s')
    ...
    print(f'[{strategy.name}] 多空选币耗时：{time.time() - s:.2f}s')
    ...
    # 6. 缓存到本地文件
    # ====================================================================================================
    file_path = conf.get_result_folder() / 'select_result.pkl'
    select_result_df[[*FACTOR_KLINE_COL_LIST, '方向', 'offset', 'target_alloc_ratio']].to_pickle(file_path)

    print(f'[{strategy.name}] 耗时: {(time.time() - s):.2f}s')
