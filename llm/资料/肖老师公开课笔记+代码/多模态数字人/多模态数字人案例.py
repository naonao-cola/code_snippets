...
config = {"configurable": {"session_id": 'zs123'}}  # 会话ID最好和用户名有关联

# chain.invoke(input=[{'role':'user', "content": '多模态大模型是什么意思？'}])
...
with gr.Blocks(title='我的数字人项目', theme=gr.themes.Soft()) as block:
    # 聊天历史记录的组件
    chatbot = gr.Chatbot(type='messages', height=500, label='我的数字人')

    # 多模态输入框
    chat_input = gr.MultimodalTextbox(
        file_types=['image', '.wav', '.mp3', '.mp4', 'docx'],
        file_count='multiple',
