/*
 * CPU GEMM 优化演进: 第四阶段 - L1/L2 Cache Tiling 与 Memory Packing
 * 参考 FLAME "How To Optimize GEMM" 教程 (MMult_4x4_13 ~ MMult_4x4_15)
 */

#define A(i,j) a[ (j)*lda + (i) ]
#define B(i,j) b[ (j)*ldb + (i) ]
#define C(i,j) c[ (j)*ldc + (i) ]

// Block sizes for L1/L2 cache tiling
#define mc 256
#define kc 128
#define nb 1000

#define min( i, j ) ( (i)<(j) ? (i): (j) )

void AddDot4x4_SSE_Packed(int, double *, int, double *, int, double *, int);
void PackMatrixA(int, double *, int, double *);
void PackMatrixB(int, double *, int, double *);
void InnerKernel(int, int, int, double *, int, double *, int, double *, int, int);

// ============================================================================
// 1. 顶层分块 (Outer Loops for Cache Tiling)
// ============================================================================
void MY_MMult( int m, int n, int k, double *a, int lda, double *b, int ldb, double *c, int ldc ) {
  int i, p, pb, ib;

  // 将大矩阵乘法划分为 mc x kc 的块，装入缓存
  for ( p=0; p<k; p+=kc ){
    pb = min( k-p, kc );
    for ( i=0; i<m; i+=mc ){
      ib = min( m-i, mc );
      InnerKernel( ib, n, pb, &A( i,p ), lda, &B( p,0 ), ldb, &C( i,0 ), ldc, i==0 );
    }
  }
}

// ============================================================================
// 2. 内部计算核 (Inner Kernel & Packing Calls)
// ============================================================================
void InnerKernel( int m, int n, int k, double *a, int lda, double *b, int ldb, double *c, int ldc, int first_time ) {
  int i, j;
  double packedA[ m * k ];
  static double packedB[ kc * nb ]; // 注意：实际工程中多线程应避免 static buffer

  for ( j=0; j<n; j+=4 ){ 
    // 仅在首次遍历 M 维时 Pack B 矩阵 (重用 L2 Cache)
    if ( first_time ) PackMatrixB( k, &B( 0,j ), ldb, &packedB[ j*k ] );
    
    for ( i=0; i<m; i+=4 ){ 
      // Pack A 矩阵以满足 _mm_load_pd 的连续内存读取要求
      if ( j==0 ) PackMatrixA( k, &A( i,0 ), lda, &packedA[ i*k ] );
      
      // 调用已 Packed 数据的 4x4 SSE 微内核
      AddDot4x4_SSE_Packed( k, &packedA[ i*k ], 4, &packedB[ j*k ], k, &C( i,j ), ldc );
    }
  }
}

// ============================================================================
// 3. Memory Packing 数据重排函数
// ============================================================================
// 将 A 的列主序片段重排为连续的 4x4 块布局
void PackMatrixA( int k, double *a, int lda, double *a_to ) {
  int j;
  for( j=0; j<k; j++){
    double *a_ij_pntr = &A( 0, j );
    *a_to++ = *a_ij_pntr;
    *a_to++ = *(a_ij_pntr+1);
    *a_to++ = *(a_ij_pntr+2);
    *a_to++ = *(a_ij_pntr+3);
  }
}

// 将 B 的列主序片段重排为连续的布局
void PackMatrixB( int k, double *b, int ldb, double *b_to ) {
  int i;
  double *b_i0_pntr = &B( 0, 0 ), *b_i1_pntr = &B( 0, 1 ), *b_i2_pntr = &B( 0, 2 ), *b_i3_pntr = &B( 0, 3 );
  for( i=0; i<k; i++){
    *b_to++ = *b_i0_pntr++;
    *b_to++ = *b_i1_pntr++;
    *b_to++ = *b_i2_pntr++;
    *b_to++ = *b_i3_pntr++;
  }
}

// ============================================================================
// 4. SSE Vectorized Inner Kernel for Packed Data
// ============================================================================
#include <xmmintrin.h>   // SSE
#include <pmmintrin.h>   // SSE2
#include <emmintrin.h>   // SSE3
#include <mmintrin.h>

typedef union {
    __m128d v;
    double  d[2];
} v2df_t;

void AddDot4x4_SSE_Packed(int k, double* a, int lda, double* b, int ldb, double* c, int ldc) {
    int p;
    v2df_t c_00_c_10_vreg, c_01_c_11_vreg, c_02_c_12_vreg, c_03_c_13_vreg;
    v2df_t c_20_c_30_vreg, c_21_c_31_vreg, c_22_c_32_vreg, c_23_c_33_vreg;
    v2df_t a_0p_a_1p_vreg, a_2p_a_3p_vreg;
    v2df_t b_p0_vreg, b_p1_vreg, b_p2_vreg, b_p3_vreg;

    c_00_c_10_vreg.v = _mm_setzero_pd(); c_01_c_11_vreg.v = _mm_setzero_pd();
    c_02_c_12_vreg.v = _mm_setzero_pd(); c_03_c_13_vreg.v = _mm_setzero_pd();
    c_20_c_30_vreg.v = _mm_setzero_pd(); c_21_c_31_vreg.v = _mm_setzero_pd();
    c_22_c_32_vreg.v = _mm_setzero_pd(); c_23_c_33_vreg.v = _mm_setzero_pd();

    for (p = 0; p < k; p++) {
        // 由于 a 和 b 已经是 packed 的连续内存，所以可以直接连续读取
        a_0p_a_1p_vreg.v = _mm_load_pd((double*)a);
        a_2p_a_3p_vreg.v = _mm_load_pd((double*)(a + 2));
        a += 4;

        b_p0_vreg.v = _mm_loaddup_pd((double*)b);       
        b_p1_vreg.v = _mm_loaddup_pd((double*)(b + 1)); 
        b_p2_vreg.v = _mm_loaddup_pd((double*)(b + 2)); 
        b_p3_vreg.v = _mm_loaddup_pd((double*)(b + 3)); 
        b += 4;

        c_00_c_10_vreg.v += a_0p_a_1p_vreg.v * b_p0_vreg.v;
        c_01_c_11_vreg.v += a_0p_a_1p_vreg.v * b_p1_vreg.v;
        c_02_c_12_vreg.v += a_0p_a_1p_vreg.v * b_p2_vreg.v;
        c_03_c_13_vreg.v += a_0p_a_1p_vreg.v * b_p3_vreg.v;

        c_20_c_30_vreg.v += a_2p_a_3p_vreg.v * b_p0_vreg.v;
        c_21_c_31_vreg.v += a_2p_a_3p_vreg.v * b_p1_vreg.v;
        c_22_c_32_vreg.v += a_2p_a_3p_vreg.v * b_p2_vreg.v;
        c_23_c_33_vreg.v += a_2p_a_3p_vreg.v * b_p3_vreg.v;
    }

    C(0, 0) += c_00_c_10_vreg.d[0]; C(0, 1) += c_01_c_11_vreg.d[0];
    C(0, 2) += c_02_c_12_vreg.d[0]; C(0, 3) += c_03_c_13_vreg.d[0];

    C(1, 0) += c_00_c_10_vreg.d[1]; C(1, 1) += c_01_c_11_vreg.d[1];
    C(1, 2) += c_02_c_12_vreg.d[1]; C(1, 3) += c_03_c_13_vreg.d[1];

    C(2, 0) += c_20_c_30_vreg.d[0]; C(2, 1) += c_21_c_31_vreg.d[0];
    C(2, 2) += c_22_c_32_vreg.d[0]; C(2, 3) += c_23_c_33_vreg.d[0];

    C(3, 0) += c_20_c_30_vreg.d[1]; C(3, 1) += c_21_c_31_vreg.d[1];
    C(3, 2) += c_22_c_32_vreg.d[1]; C(3, 3) += c_23_c_33_vreg.d[1];
}
