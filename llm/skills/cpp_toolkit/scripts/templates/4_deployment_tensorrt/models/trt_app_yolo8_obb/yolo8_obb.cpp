...
#include "../../../include/private/trt/trt_app_yolo8_obb/yolo8_obb.hpp"
#if 1
namespace yolo8_obb {
    ...
    std::tuple<float, float, float> convariance_matrix(const Box& obb){
        float w = obb.width;
        float h = obb.height;
        float r = obb.angle;
        float a = w * w / 12.0;
    ...
    static float probiou(const Box& obb1, const Box& obb2, float eps = 1e-7){
        // Calculate the prob iou between oriented bounding boxes, https://arxiv.org/pdf/2106.06072v1.pdf.
        float a1, b1, c1, a2, b2, c2;
        tie(a1, b1, c1) = convariance_matrix(obb1);
        tie(a2, b2, c2) = convariance_matrix(obb2);
                ...
                for(int ibatch = 0; ibatch < infer_batch_size; ++ibatch){
                    ...
                    for(int i = 0; i < count; ++i){
                        ...
                        if(keepflag == 1){
                            ...
                            //image_based_boxes.emplace_back(pbox[0], pbox[1], pbox[2], pbox[3], 1, label);
                        }
                    }
...
namespace yolo8_obb
{
    ...
    public:
        ...
        virtual bool startup(const string& file, int gpuid, float confidence_threshold, float nms_threshold, NMSMethod nms_method, int max_objects, bool use_multi_preprocess_stream)
        {
        ...
        virtual void worker(promise<bool>& result) override
        {
            ...
            auto output      = engine->tensor(engine->get_output_name(0));  //"output"
            int  num_classes = output->size(1) - 5;
            ...
            while (get_jobs_and_wait(fetch_jobs, max_batch_size)) {
                ...
                for (int ibatch = 0; ibatch < infer_batch_size; ++ibatch) {
                    ...
                    auto   affine_matrix      = affin_matrix_device.gpu<float>(ibatch);
                    checkCudaRuntime(cudaMemsetAsync(output_array_ptr, 0, sizeof(int), stream_));
                    decode_kernel_invoker(image_based_output, output->size(2), num_classes, confidence_threshold_, affine_matrix, output_array_ptr, MAX_IMAGE_BBOX, stream_);
                    if (nms_method_ == NMSMethod::FastGPU) {
                        nms_kernel_invoker(output_array_ptr, nms_threshold_, MAX_IMAGE_BBOX, stream_);
                    }
                }
                ...
                for (int ibatch = 0; ibatch < infer_batch_size; ++ibatch) {
                    ...
                    for (int i = 0; i < count; ++i) {
                        ...
                        int    keepflag = pbox[7];
                        if (keepflag == 1) {
                            Box box(pbox[0], pbox[1], pbox[2], pbox[3], pbox[4], pbox[5],label);
                            
                            image_based_boxes.emplace_back(box);
                            
                            Box box1(pbox[0], pbox[1], pbox[2], pbox[3], pbox[4], label);
                            image_based_boxes.emplace_back(box1);
                        }
                    }
        ...
        virtual std::shared_future<BoxArray> 
        commit(const Mat& image) override
        {
            return ControllerImpl::commit(image);
        }
    ...
    shared_ptr<Infer> create_infer(const string& engine_file, int gpuid, float confidence_threshold, float nms_threshold, NMSMethod nms_method, int max_objects, bool use_multi_preprocess_stream)
    {
        shared_ptr<InferImpl> instance(new InferImpl());
        if (!instance->startup(engine_file, gpuid, confidence_threshold, nms_threshold, nms_method, max_objects, use_multi_preprocess_stream)) {
            instance.reset();
        }
        return instance;
    }
