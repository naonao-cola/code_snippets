/*
 * TensorRT 自定义插件 (Plugin) CUDA 实现范式
 * 提取自 cuda-sample/trt_mode/plugin/custom_scalar_plugin.cu
 */

#include <cuda_runtime.h>
#include <math.h>

// 1. CUDA 核函数实现 (GridSample 的双线性插值)
...
// 2. Kernel 启动器封装 (供 Plugin 的 enqueue() 方法调用)
void customScalarImpl(const float *input, const float *grid, float *output,
  ...
  // 注意：务必传入 stream 参数，与 TRT 推理流保持同步
