...
FrontPinDetect::FrontPinDetect()
{
	...
	selectModel["rxwm_online"] = productNames::rxwm_online;
...
AlgoResultPtr FrontPinDetect::RunAlgo(InferTaskPtr task, std::vector<AlgoResultPtr> pre_results)
{
	...
	// 是否存在csv坐标文件
	std::string csv_path = Tival::JsonHelper::GetParam(params["param"], "csv_path", std::string(""));
	ParsingCSVCoord(csv_path);

	cv::Mat drawImage = cv::Mat::zeros(task->image.size(), task->image.type());
	task->image.copyTo(drawImage);

	cv::Mat img1_gray, img2_gray;
	cv::cvtColor(task->image, img1_gray, cv::COLOR_BGR2GRAY);   // 同轴光图片
	cv::cvtColor(task->image2, img2_gray, cv::COLOR_BGR2GRAY);  // 平行光图片
	double angle = 0;
	cv::Mat roi_mask;
	//
	...
	if (enable_save_resultImage) {
		...
		if (state) {
			cv::imwrite(rImage_Ori + "/" + std::string(task->image_info["img_name"]) + "_white.png", task->image);
			cv::imwrite(rImage_Ori + "/" + std::string(task->image_info["img_name"]) + "_red.png", task->image2);
		}
	}
	...
	if (coordinate_x.size()) {
		expect_pin_count = std::accumulate(coordinate_x.begin(), coordinate_x.end(), 0, [](int sum, const std::vector<float>& inner_vec) {
...
void FrontPinDetect::FirstPassLocate(const cv::Mat& redImage, const cv::Mat& whiteImage, const json& params, cv::Mat& bin_image, std::vector<PinInfoPtr>& pin_infos, std::string image_name, bool draw_contours)noexcept
{
	...
	case productNames::rxwm_small:
		...
		break;
	case productNames::rxwm_online:
		svm_obj._train_size = cv::Size(80, 80);
		isInit = svm_obj.init("./", "rxwm_online.mdl");
		modelName = "rxwm_online.mdl";
		break;
	// ------------柔性直公----------------//
	...
	/*	cv::Mat drawImage = redImage.clone();
		...
		cv::Mat sub_SvmImg = whiteImage(cropBox).clone();
			...
			for (auto cont2 : contorus2)
			{
			   ...
			   // double area = cv::contourArea(cont2);
				...
				if (x_off < cent_off_thresh_x && y_off < cent_off_thresh_y
					...
					cur_pin_coord = cv::Point2i(round(center.x + pin_info->bbox.x + roi_lt_x), round(center.y + pin_info->bbox.y + roi_lt_y));
					cur_pin_cont = cont2;
					cur_pin_area = area;
...
cv::Mat FrontPinDetect::GetPinCSTransMat(const std::vector<PinInfoPtr>& pin_infos, const json& params, double roiAngle, cv::Mat& img_draw)
{
	...
	bool fit_by_one = Tival::JsonHelper::GetParam(params, "fit_by_one", false);
	...
	int rows_vec, cols_vec;
	if (coordinate_x.size()) {
		y_coords1 = coordinate_y[1];
		y_coords2 = coordinate_y[0];
		x_coords1 = coordinate_x[1];
		x_coords2 = coordinate_x[0];
		rows_vec = coordinate_y.size();
		cols_vec = coordinate_y[0].size();
	}
	else{
		y_coords1 = params["y_coords1"];       // Y坐标值
		y_coords2 = params["y_coords2"];
		x_coords1 = params["x_coords1"];
		x_coords2 = params["x_coords2"];
		rows_vec = int(y_coords1.size() + y_coords2.size());
		cols_vec = int(std::max(x_coords1.size(), x_coords2.size()));
	}
	...
	while (angle_ratio > 1.15) {
		...
		std::vector<std::vector<cv::Point>> pin_rows(rows_vec + 5);
		std::vector<std::vector<cv::Point>> pin_cols(cols_vec + 5);
		//行高
		...
		for (auto pin : pin_infos) {
			...
			int min_area_row = row_height_px * 0.5;
			int min_area_col = col_width_px * 0.5;
...
json FrontPinDetect::CalcPinResults(std::vector<PinInfoPtr>& pin_infos, const cv::Mat& M, const cv::Mat img1_gray, const cv::Mat img2_gray, const cv::Mat bin_img2, const json& params, double& adj_x, double& adj_y, int roi_lt_x, int roi_lt_y, float& differ_std, float& differ_measure)
{
	...
	json y_coords2;
	if (coordinate_x.size()) {
		x_coords2 = coordinate_x;   // X坐标值
		y_coords2 = coordinate_y;   // Y坐标值
	}
	else {
		x_coords1 = params["x_coords1"];   // 基数行X坐标
		x_coords2 = params["x_coords2"];   // 偶数行X坐标
		y_coords1 = params["y_coords1"];   // 基数行Y坐标值
		y_coords2 = params["y_coords2"];   // 偶数行Y坐标值
	}
	...
	int pin_index = -1;
	int Rows_Sum = 0;
	std::vector<int> Cols_Size;
	if (coordinate_x.size()) {
		Rows_Sum = coordinate_x.size();
		Cols_Size = { int(coordinate_x[0].size()), int(coordinate_x[1].size()) };
	}
	else {
		Rows_Sum = y_coords1.size() + y_coords2.size();
		Cols_Size = { int(x_coords2.size()), int(x_coords1.size()) };	
	}
	int result_state = 0;
	...
	for (int i = 0; i < Rows_Sum; i++) {
		...
		for (int j = 0; j < Col_Sum; j++) {
			...
			double y_coord;
			if (coordinate_x.size()) {
				x_coord = coordinate_x[i][j];
				y_coord = coordinate_y[i][j];
			}
			else {
				// 区分奇偶数行
				x_coord = i % 2 == 0 ? x_coords2[j] : x_coords1[j];
				y_coord = i % 2 == 0 ? y_coords2[i / 2] : y_coords1[i / 2];
			}
...
json FrontPinDetect::CalcStdLines(const cv::Mat& M, const json& params, double adj_x, double adj_y, float differ_std)
{
	...
	if (coordinate_x.size()) {
		x_coords2 = coordinate_x[0];
		y_coords2 = coordinate_y[coordinate_y.size()-1];
		end_x = x_coords2[x_coords2.size() - 1];
		end_y = y_coords2[0];
	}
	else {
		x_coords1 = params["x_coords1"];
		x_coords2 = params["x_coords2"];
		y_coords1 = params["y_coords1"];
		y_coords2 = params["y_coords2"];

		end_x = x_coords2[x_coords2.size() - 1];
		end_y = y_coords2[y_coords2.size() - 1];
	}
	...
	// X\Y 轴，标线两端延长500px
	cv::Point2f axis_x_end = cv::Point2f(Mm2Px(end_x, ppum) + 500, 0);
	cv::Point2f axis_y_end = cv::Point2f(0, Mm2Px(end_y, ppum) + 500);							// 不偏移

	//对gap进行y方向整体偏移（用于针尖不在同一条线上）
	if (enable_offset){
		origin = cv::Point2f(Mm2Px(differ_std, ppum), -Mm2Px(gap, ppum));										// 偏移（x，y轴起点x坐标）
		axis_x_end = cv::Point2f(Mm2Px(end_x, ppum) + 500, -Mm2Px(gap, ppum));						// 偏移（X轴终点y坐标）
		axis_y_end = cv::Point2f(Mm2Px(differ_std, ppum), Mm2Px(end_y, ppum) + 500);  // 偏移（y轴终点x坐标）
	}
...
bool FrontPinDetect::ParsingCSVCoord(std::string& csv_path)
{
	if (csv_path == "") {
		LOGE("csv_path is null")
		return false;
	}
	std::ifstream file(csv_path);  
	if (!file.is_open()) {
		LOGE("csv fill unable to open")
		return false;
	}
	...
	while (std::getline(file, line)) {
		...
		// 存入坐标组
		if (count % 2 == 0) {
			coordinate_x.push_back(row);
		}
		else {
			coordinate_y.push_back(row);
		}
		count++;
	}

	for (int i = 0; i < coordinate_x.size(); i++) {
		if (coordinate_x[i].size() != coordinate_y[i].size()) {
			LOGE("csv x y num not equal");
			coordinate_x = {};
			coordinate_y = {};
			return false;
		}
