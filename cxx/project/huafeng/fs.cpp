...
namespace nao {
/**
 * @FilePath     : /connector_algo/src/custom/fs.cpp
 * @Description  :
 * @Author       : naonao
 * @Date         : 2024-07-19 15:37:28
 * @Version      : 0.0.1
 * @LastEditors  : naonao
 * @LastEditTime : 2024-07-19 15:37:29
**/
	namespace fl {
		/*
		...
		*/
		...
		uintmax_t getFileSize(const std::string& filename) {
			if (!fs::exists(filename)) return -1;
		...
		void getFileList(const std::string& filePath, const int& lable, const std::string& distAll, const std::string& format) {
			...
			if (format != "") getAllFormatFiles(filePath, files, format);
			if (format == "") getAllFiles(filePath, files);
		...
		int alterFileName(const std::string& filename, const std::string& newname) {
			if (!fs::exists(filename)) return -2;
			try {
		...
		bool filterField(const std::vector<std::string>& srcStringList, std::vector<std::string>& dstStringList,
			...
			if (srcStringList.size() <= 0 || format.size() <= 0) return false;
		...
		bool filterField(const std::vector<std::string>& srcStringList, std::vector<std::string>& dstStringList, const std::string& format) {
			if (srcStringList.size() <= 0 || format.empty()) return false;
		...
		file::operator bool() const {
			if (_fout.is_open() || _fin.is_open()) return true;
			return false;
		}
		...
		void file::seek(int64_t off, int whence) {
			...
			if (_fin.is_open())_fin.seekg(off, whence);
		}

		size_t file::read(void* s, size_t n) {
			if (!_fin.is_open()) return 0;
			...
			//判断读取大小是否超限
			{
				...
				if (n > (size_t)(remain - cur_pos)) n = remain - cur_pos;
				if (n == 0) return 0;
				_fin.seekg(cur_pos);
			}
			_fin.read((char*)s, n);
