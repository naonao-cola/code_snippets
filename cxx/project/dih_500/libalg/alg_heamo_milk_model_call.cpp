...
bool view_pair_idx_flag = false;
int  view_pair_idx_old  = 0;
                                           ...
                                           const std::map<std::string, float>& call_back_params)
{
    ...
    if (view_pair_idx == 0) {
        if (!view_pair_idx_flag) {
            view_pair_idx_flag = true;
            g_run_count++;
        }
    }
    else if (view_pair_idx != view_pair_idx_old) {
        g_run_count++;
        view_pair_idx_old  = view_pair_idx;
        view_pair_idx_flag = false;
    }
    else {
        ALGLogInfo << "当前传入的view_pair_idx： " << view_pair_idx << "   当前的计数数量为： " << view_pair_idx_old << "\n";
    }
}
