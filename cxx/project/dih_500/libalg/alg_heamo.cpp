...
std::vector<AiMap_t> heamo_cnt_maplist_human = {
    ...
    AI_MAP_DEF(HeamoCnt_t, PV4, "PV4", 0.1),         AI_MAP_DEF(HeamoCnt_t, PV5, "PV5", 0.1),         AI_MAP_DEF(HeamoCnt_t, PV6, "PV6", 0.1),
    AI_MAP_DEF(HeamoCnt_t, MIC_FLU, "MIC_FLU", 0.1),
};
...
std::vector<AiModReg_t> heamo_mod_reglist_human = {
    ...
    AI_MOD_DEF(NNET_MODID_FLUMIC, "MICFLU", 1, 1, NNET_GROUP_HUMAN, LETTERBOX, 1, 1, 1, 18, 1, 0, 0),             // 荧光微球计数模型
};
...
/* 荧光微球视图注册表 */
std::vector<AiViewReg_t> flumic_view_reglist = {
    AI_VIEW_DEF(AI_VIEW_TYPE_BRI, NNET_MODID_FLUMIC, NNET_GROUP_HUMAN | NNET_GROUP_DOG | NNET_GROUP_CAT, 0.25),
    AI_VIEW_DEF(AI_VIEW_TYPE_FLU, NNET_MODID_FLUMIC, NNET_GROUP_HUMAN | NNET_GROUP_DOG | NNET_GROUP_CAT, 0.25)};
...
std::vector<AiChlReg_t> heamo_human_chl_reglist = {
    ...
    AI_CHL_DEF(AI_CHL_TYPE_FLU_MIC, "荧光微球", flumic_view_reglist, Heamo_ImgFluMicChl, NNET_GROUP_HUMAN),
...
static std::vector<std::string> g_heamo_model_name  = {};
static int                      g_heamo_model_count = 0;

int dynamic_gen_model(std::vector<AiModReg_t>& reglist, tinyxml2::XMLElement* root_element, const char* type, nlohmann::json& j)
{
    reglist.clear();
    if (std::string(root_element->Value()) == "ModelSetting") {
        ...
        for (tinyxml2::XMLElement* iter = root_element->FirstChildElement("DetectType"); iter; iter = iter->NextSiblingElement("DetectType")) {
            ...
            if (strcmp(item_attribute_name, type) == 0) {
                ALGLogInfo << "================================================================================= \r\n";
                ALGLogInfo << "Element Attribute name : " << item_attribute_name << "\r\n";
                for (tinyxml2::XMLElement* model_iter = iter->FirstChildElement("model"); model_iter;
                     model_iter                       = model_iter->NextSiblingElement("model")) {
                    ...
                    g_heamo_model_name.push_back(mod_name_str);
                    ...
                    j.push_back(g_heamo_model_name[g_heamo_model_count].c_str());
                    reglist.push_back(AI_MOD_DEF(static_cast<NNetModID>(mod_id),
                                                 g_heamo_model_name[g_heamo_model_count].c_str(),
...
int Heamo_DynamicGenMod(const char* cfg_path, AlgCellModeID_e mode_id,std::string& model_info)
{

    std::string clarity_path = std::string().append(cfg_path);
    clarity_path.append("/alg/model_cfg.xml");

    std::cout << "Heamo_DynamicGenMod 模型路径： " << clarity_path << std::endl;
    tinyxml2::XMLDocument doc;
    tinyxml2::XMLError    ret = doc.LoadFile(clarity_path.c_str());
    if (ret != 0) {
        ALGLogError << "fail to load xml file: " + clarity_path << std::endl;
        return -1;
    }
    ...
    if (mode_id== 1) {
        ...
        dynamic_gen_model(heamo_mod_reglist_human, root_element, "heamo_human", ret_json["GROUP_HUMAN"]);
    }
    if (mode_id == 2) {
        ret_json["GROUP_CAT"] = {};
        ret_json["GROUP_DOG"] = {};
        tinyxml2::XMLElement* root_element = doc.RootElement();
        dynamic_gen_model(heamo_mod_reglist_cat, root_element, "heamo_cat", ret_json["GROUP_CAT"]);
        dynamic_gen_model(heamo_mod_reglist_dog, root_element, "heamo_dog", ret_json["GROUP_DOG"]);
    }
    if (mode_id == 3) {
        ret_json["GROUP_MILK"] = {};
        tinyxml2::XMLElement* root_element = doc.RootElement();
        dynamic_gen_model(heamo_mod_reglist_milk, root_element, "heamo_milk", ret_json["GROUP_MILK"]);
    }
...
int Heamo_OpenFlagCheck(HeamoCtxID_t ctx_id, const HEAMO_OPEN_TYPE& open_type, bool& open_flag_sample_first_time)
{
    ...
    std::cout << " open_type " << open_type << " cur_open_type " << cur_open_type << std::endl;
    // 禁止重复close
                          ...
                          const bool&               open_flag_sample_first_time)
{
    ...
    if (open_flag_sample_first_time) {
        ...
        std::cout << " Ai_FindMaxChannelNums " << max_channel_nums << std::endl;
...
int Heamo_PushImage(HeamoCtxID_t           ctx_id,
                    std::list<HeamoImg_t>& img_list,
                    uint32_t               group_idx,
                    uint32_t               chl_idx,
                    ...
                    int                    z)
{
    ...
    if (chl_idx == 1 && ctx->cnt.WBC > g_alarm_param.wbc_num) {
        ALGLogInfo << "Heamo_PushImage 当前检测图片满足要求,不需要在推入图片"<< "\n";
        ALGLogInfo << "Heamo_PushImage 当前的rbc计数: " << ctx->cnt.RBC << "  当前的wbc计数: " << ctx->cnt.WBC
                   << " rbc 阈值:  " << g_alarm_param.rab_num << " wbc 阈值:" << g_alarm_param.wbc_num<< "\n";
        return 11;
    }
    if (ctx->cnt.RBC > g_alarm_param.rab_num && chl_idx ==0) {
        ALGLogInfo << "Heamo_PushImage 当前检测图片满足要求,不需要在推入图片" << "\n";
        ALGLogInfo << "Heamo_PushImage 当前的rbc计数: " << ctx->cnt.RBC << "  当前的wbc计数: " << ctx->cnt.WBC
                   << " rbc 阈值:  " << g_alarm_param.rab_num << " wbc 阈值:" << g_alarm_param.wbc_num << "\n";
        return 11;
    ...
    std::cout << "Heamo_PushImage 血球开始推入图片,推入的组： " << group_idx << " 通道数 " << chl_idx << " 视图 " << view_pair_idx << "\n";


    AiChlReg_t* chl = Heamo_FindChl(Heamo_FindGroup(group_idx), chl_idx);
    if (chl == NULL) {
        ALGLogError << "Heamo_PushImage 血球推入图片失败, 通道为空" << "\n";
        std::cout << "Heamo_PushImage 血球推入图片失败, 通道为空" << "\n";
        return -2;
    }
    ...
    if (Heamo_GetViewReglist(view_list, group_idx, chl_idx)) {
        ...
        std::cout << "Heamo_PushImage 血球推入图片失败, 视图为空" << "\n";
        return -3;
    }
    ...
    if (!view_list.size() || img_list.size() < view_list.size()) {
        ALGLogError << "Heamo_PushImage 血球推入图片失败, 参数错误，图片数量与视图数量不符,图片数量  " << img_list.size() << " 视图数量  "
                    << view_list.size() << "\n";
        std::cout << "Heamo_PushImage 血球推入图片失败, 参数错误，图片数量与视图数量不符,图片数量  " << img_list.size() << " 视图数量  "
                  << view_list.size() << "\n";
        return -4;
    }


    std::map<std::string, float> call_back_params{{TASK_TYPE, TASK_TYPE_HEAMO}, {"CHECK", checkout_flag}, {"X", x}, {"Y", y}, {"Z", z}};
    HEAMO_CTX_GROUP_IDX(ctx) = group_idx;
    std::cout << "Heamo_PushImage 1"<< std::endl;
    std::cout << "Heamo_PushImage 血球推入图片,推入的组： " << group_idx << " 通道数 " << chl_idx << " 视图 " << view_pair_idx << "\n"
              << std::endl;
    std::cout << " ctx->cnt.channel_img_nums " << ctx->cnt.channel_img_nums.size() << std::endl;
    ...
    if (!Ai_ItemPush(HEAMO_CTX_AI_CTXID(ctx),
        ...
        std::cout << "Heamo_PushImage 血球推入图片失败,推入的组： " << group_idx << " 通道数 " << chl_idx << " 视图 " << view_pair_idx << "\n"
                  << std::endl;
        return -6;
    }
    std::cout << "Heamo_PushImage 2" << std::endl;
                   ...
                   bool                    qc = false)
{
    ...
    for (int idx = 0; idx < channel_list.size(); ++idx) {
        ...
        default:
        {
            ALGLogError << "Unknown channel type is found in group : channel " << inner_group_idx << " : " << idx;
            break;
        }
        }
    }
...
int Heamo_Close(HeamoCtxID_t ctx_id)
{
    ...
    if (ret) {
        ALGLogError << "Heamo_Close 打印关闭参数 失败 \n";
        return ret;
    }
