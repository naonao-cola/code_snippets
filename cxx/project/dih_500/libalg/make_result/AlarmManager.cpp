...
int HeamoAlarmManager::SetCustomAlarmParams(const std::vector<float>& alarm_params_v){
  ...
  wbc_num_high_thr=alarm_params_v[0];
  wbc_num_low_thr=alarm_params_v[1];

  neu_num_high_thr=alarm_params_v[2];
  neu_num_low_thr=alarm_params_v[3];

  lym_num_high_thr=alarm_params_v[4];

  mon_num_high_thr=alarm_params_v[5];

  eos_num_high_thr=alarm_params_v[6];

  baso_num_high_thr=alarm_params_v[7];

  ig_num_upper_thr=alarm_params_v[8];

  // 白细胞unknown,白细胞 个数
  wbc_num_count_1          = alarm_params_v[9];
  wbc_unknown_percentage = alarm_params_v[10];
  wbc_num_count_2        = alarm_params_v[11];

  rbc_rdw_sd_upper_thr=alarm_params_v[12];
  rbc_rdw_cv_upper_thr=alarm_params_v[13];
  rbc_mcv_low_thr=alarm_params_v[14];
  rbc_mcv_high_thr=alarm_params_v[15];
  rbc_num_high_thr=alarm_params_v[16];
  rbc_hgb_low_thr=alarm_params_v[17];
  rbc_mchc_low_thr=alarm_params_v[18];
  plt_num_high_thr=alarm_params_v[19];
  plt_num_low_thr=alarm_params_v[20];
  ret_num_high_thr=alarm_params_v[21];
  ret_num_low_thr=alarm_params_v[22];
  // 获取pdw的报警参数
  pdw_low_thr  = alarm_params_v[23];
  pdw_high_thr = alarm_params_v[24];



  return 0;
}
int HeamoAlarmManager::GetAlarmResults(const float&              wbc,
                                       const float&              neu,
                                       const float&              lym,
                                       const float&              mono,
                                       const float&              eos,
                                       ...
                                       const float&              nrbc,
                                       const float&              ig,
                                       const float&              wbc_agglomeration,   // wbc
                                       const std::vector<float>& rbc_curve,
                                       const float&              rdw_cv,
                                       const float&              rdw_sd,
                                       const float&              mcv,
                                       const float&              rbc,
                                       const float&              hgb,
                                       const float&              mchc,
                                       const float&              rbc_agglomeration,   // rbc
                                       const float&              plt,
                                       const std::vector<float>& plt_curve,
                                       const float&              plt_agglomeration,   // plt
                                       const float&              ret,                 // ret
                                       const float&              pdw,
                                       const float&              unknown_percentage,
                                       const int&               wbc_count,
                                       std::vector<std::string>& output)
{
    ...
    Heamo_AlarmCheckSingleNormal(
        wbc_agglomeration, ALARM_ALL_PSEUDO_MIN_THR, wbc_aggregation_num_upper_thr, "wbc_aggregation_num_upper_warning", output);

    Heamo_AlarmCheckSingleNormal(pdw, pdw_low_thr, 1.0f, "pdw_num_low_warning", output);
    Heamo_AlarmCheckSingleNormal(pdw, 0.f, pdw_high_thr, "pdw_num_high_warning", output);

    // RBC

    Heamo_AlarmCheckSingleNormal(rdw_sd, ALARM_ALL_PSEUDO_MIN_THR, rbc_rdw_sd_upper_thr, "rbc_rdw_sd_upper_warning", output, true);
    Heamo_AlarmCheckSingleNormal(rdw_cv, ALARM_ALL_PSEUDO_MIN_THR, rbc_rdw_cv_upper_thr, "rbc_rdw_cv_upper_warning", output, true);
    ...
    Heamo_AlarmCheckSingleNormal(rbc_agglomeration, ALARM_ALL_PSEUDO_MIN_THR, rbc_aggregation_num_upper, "rbc_aggregation_num_upper_warning", output);
    // 双峰检测
    ...
    Heamo_AlarmCheckSingleNormal(
        plt_agglomeration, ALARM_ALL_PSEUDO_MIN_THR, plt_aggregation_num_upper_thr, "plt_aggregation_num_upper_warning", output);
    // RET
    ...
    //TODO 白细胞unknown 报警
    Heamo_AlarmCheckSingleNormal(unknown_percentage, ALARM_ALL_PSEUDO_MIN_THR, wbc_unknown_percentage, "wbc_uk_percentage_warning", output);
    Heamo_AlarmCheckSingleNormal(wbc_count, wbc_num_count_1, ALARM_ALL_PSEUDO_MAX_THR, "wbc_num_cls_warning", output);
    Heamo_AlarmCheckSingleNormal(wbc_count, wbc_num_count_2, ALARM_ALL_PSEUDO_MAX_THR, "wbc_num_net_warning", output);
...
int HeamoAlarmManager::ModifyAlarmResults(const std::vector<std::string>& previous_alarm,
                                          const float&                    wbc,
                                          const float&                    neu,
                                          const float&                    lym,
                                          const float&                    mono,
                                          ...
                                          const float&                    ret,                  // ret
                                          const float&                    pdw,                  // pdw
                                          const float&                    unknown_percentage,   // pdw
                                          const int&                      wbc_num,   // pdw
                                          std::vector<std::string>&       output)
{
    output.clear();
    // 根据下发参数填入无法计算的参数
    ...
    GetAlarmResults(wbc,
                    ...
                    hgb,
                    mchc,
                    rbc_agglomeration,   // rbc
                    plt,
                    plt_curve,
                    plt_agglomeration,    // plt
                    ret,                  // ret
                    pdw,                  // pdw
                    unknown_percentage,   // unknown wbc
                    wbc_num,
                    output);
    return 0;
}
