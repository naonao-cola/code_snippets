...
#define ALGCELL_VERSION "V1.0.15.20250806"

extern bool g_pla_flag;
extern bool g_cbc_flag;
int g_machine_type;
...
#define HEAMO_AI_ITEM_NUMS   6
#define CLARITY_AI_ITEM_NUMS 1
...
{
    ALGLogInfo << "传入的机型版本: " << machine_type;
    if (machine_type != 1 && machine_type != 2 && machine_type!=3) {
      //机型只能是 1  2  3 低端  中段 高端
      return nullptr;
    }
    else{
...
static uint8_t *Alg_ReadFile(const char *path, const char *keyword, const char *suffix, uint32_t *len) {
  ...
  ALGLogInfo << "传入的路径： " << path;
  ALGLogInfo << "传入的关键词：  " << keyword;
  ALGLogInfo << "传入的后缀： " << suffix;
  ALGLogInfo<<"Model path "<<filename;
  std::cout << "Alg_ReadFile filename: " << filename << std::endl;
...
Alg_SyncModFile(AlgCtxID_t ctx_id, const char *cfg_path, AiGroupReg_t &group, AiModReg_t &mod, const int &ai_type) {
  ...
  uint8_t* mod_data   = Alg_ReadFile(fullpath.data(), mod.name, "rknn", &mod_size);
  //	uint8_t *label_data = Alg_ReadFile(fullpath.data(), AI_MOD_NAME(&mod), "txt", &label_size);
  std::cout << "model fullpath:" << fullpath << std::endl;
  if (mod_data == nullptr) {
  ...
  std::cout << " 302 group.group_id " << group.group_id << " mod.mod_id " << mod.mod_id << std::endl;
  //在此处传入读取的参数及要求参数个数
  int ret = Ai_AddModel(
      specific_ai_ctx,
      group.group_id,
  ...
  if (ret == 0) {
      ALGLogError << "succeed to add " << AI_GROUP_CFG_PATH(&group) << " " << AI_MOD_NAME(&mod) << " " << mod_size << " " << label_size;
  } else {
...
static int Alg_ConfigHeamo(AlgCtxID_t ctx_id, AlgCellModeID_e mode_id, const char* cfg_path, std::string& model_info)
{
    ...
    Heamo_DynamicGenMod(cfg_path, mode_id, model_info);
    // 依次对每个组进行初始化
...
static int Alg_ConfigClarity(AlgCtxID_t ctx_id, AlgCellModeID_e mode_id, const char* cfg_path, std::string& model_info)
{
    std::vector<AiGroupReg_t> group_list;
    ...
    // 动态构建清晰度模型注册表
    Clarity_DynamicGenMod(cfg_path, model_info);
    //  依次对每个组进行初始化
    for (const auto& group_idx : group_id_v) {
        std::vector<AiModReg_t> mod_list;
        // 获取当前组包含的模型列表
        ...
        for (auto& mod : mod_list) {
            ALGLogInfo << "Add model " << AI_MOD_NAME(&mod);
            std::cout << "Add model " << AI_MOD_NAME(&mod) << std::endl;
...
* @return 0 success other fail
 */
int AlgCell_RunConfigLoad(AiCtxID_t ctx_id, AlgCellModeID_e mode_id, const char* cfg_path, std::vector<char>& ret_model_info)
{
    AlgCtx_t* ctx = (AlgCtx_t*)ctx_id;
    ...
    std::cout << "传入的模型路径： " << cfg_path<<std::endl;
    std::string model_info_heamo;
    if (Alg_ConfigHeamo(ctx_id, mode_id, cfg_path, model_info_heamo)) {
        ALGLogError << "failed to init machine " << mode_id;
        AlgCell_RunConfigUnload(ctx_id);
        return -3;
    }

    if (Alg_ConfigClarity(ctx_id, mode_id, cfg_path, model_info_heamo)) {
        ALGLogError << "failed to init machine " << mode_id;
        AlgCell_RunConfigUnload(ctx_id);
        return -4;
    }
    ALGLogInfo << "model_info" << model_info_heamo;

    ret_model_info.assign(model_info_heamo.begin(), model_info_heamo.end());
...
int AlgCell_ParseOpenParams(const std::map<std::string, std::vector<float>>& open_params,
  ...
  g_pla_flag = static_cast<bool>(pla_v[0]);
    ...
    // 五分类信息显示
  std::vector<float> cbc_v;
  if (ParseOneParams(open_params, OPEN_PARAM_CBC, cbc_v)) {
      return ALG_ERR_PARSE_OPEN_PARAM;
  }
  g_cbc_flag = static_cast<bool>(cbc_v[0]);
...
int AlgCell_HeamoOpen(AlgCtxID_t ctx_id, uint32_t func_mask, AlgCellImageCallback_f callback, void *userdata, const std::map<std::string, std::vector<float>>& open_params) {
  ...
  for(const auto& param_iterm:open_params){
    ALGLogInfo << "AlgCell_HeamoOpen open_params key: " << param_iterm.first << " value: ";
    for(const auto& value_item:param_iterm.second){
        ALGLogInfo << value_item << " ";
    }
    ALGLogInfo << "\n";
  }
  std::cout << "AlgCell_HeamoOpen ret AlgCell_ParseOpenParams " << ret << std::endl;
  if (ret) {
...
AlgCell_HeamoPushImage(AlgCtxID_t ctx_id, std::vector<AlgCellImg_t> &image_list, uint32_t group_idx, uint32_t chl_idx,
  ...
  std::cout << "接收到图片数: " << image_list.size() << std::endl;
  ...
  for (uint32_t idx = 0; idx < image_list.size(); idx++) {
    ...
    if (img->data ==NULL){
        std::cout << "Heamo_AddImgList 图片为空 " << std::endl;
        return -3;
    }
    if (Heamo_AddImgList(ALGCELL_CTX_HEAMO_CTXID(ctx),
                          data_list,
                          group_idx,
                          chl_idx,
                          idx,
                          ALGCELL_IMG_DATA(img),
                          ALGCELL_IMG_WIDTH(img),
  ...
  auto checkout_flag_iter             = complementary_params.find("CHECK");

  auto x_iter = complementary_params.find("X");
  auto y_iter = complementary_params.find("Y");
  auto z_iter = complementary_params.find("Z");

  if (complementary_params_iter != complementary_params.end()) {
    const int &view_pair_idx = (int) complementary_params_iter->second;
    int  checkout_flag = checkout_flag_iter != complementary_params.end()? (int) checkout_flag_iter->second : 0;

    int x = x_iter != complementary_params.end() ? x_iter->second : 0;
    int y = y_iter != complementary_params.end() ? y_iter->second : 0;
    int z = z_iter != complementary_params.end() ? z_iter->second : 0;

    std::cout << "view_pair_idx 参数开始 " << view_pair_idx << std::endl;
    std::cout << "checkout_flag 参数开始 " << checkout_flag << std::endl;
    std::cout << "x_iter 参数开始 " << x << std::endl;
    std::cout << "y_iter 参数开始 " << y << std::endl;
    std::cout << "z_iter 参数开始 " << z << std::endl;
    return Heamo_PushImage(ALGCELL_CTX_HEAMO_CTXID(ctx), data_list, group_idx, chl_idx, view_pair_idx, checkout_flag,x,y,z);
  }
...
int AlgCell_HeamoGetCount(bool zero_flag, int64_t& nms_time)
{
    ...
    return HeamoGetCount(zero_flag,nms_time);
}

int AlgCell_HeamoGetCurrentCellCount(AlgCtxID_t ctx_id, CurrentCount_t& current_count){
    AlgCtx_t* ctx = (AlgCtx_t*)ctx_id;
    HeamoGetCurrentCellCount(ALGCELL_CTX_HEAMO_CTXID(ctx), current_count);
}
...
int AlgCell_HeamoGetError(Passageway_t& passageway_info)
{
    ...
    return HeamoGetError(passageway_info);
}
...
void PrintHeamoResult(const AlgCellRst &result) {
    std::cout << " 917 ALG RESULT 机型: "<<g_machine_type;
    ALGLogInfo << "ALG RESULT 机型: "<<g_machine_type;
    ALGLogInfo << "ALG RESULT >>>\r\n序号  名称        单位        数值        OPEN_FLAG\r\n";
    // 血球
    uint32_t idx = 1;
    for (const auto& item : result.heamo) {
        ALGLogInfo << std::setw(10) << idx++ << std::setw(10) << item.name.data() << std::setw(10) << item.value << std::setw(10) << item.unit.data()
                   << std::setw(10) << item.open_flag;
  ...
  if (!result.curve_rbc.empty()) {
      std::cout << "rbc curve size: " << result.curve_rbc.size();
  ...
  if (!result.curve_plt.empty()) {
      std::cout << "plt curve size: " << result.curve_plt.size();
...
int AlgCell_HeamoGetResult(AlgCtxID_t ctx_id, AlgCellRst_t& result, uint32_t timeout)
{
    ...
    if (ALGCELL_CTX_FUNC_MASK(ctx) & ALGCELL_FUNC_HEAMO) {
        ...
        ret = Heamo_GetResult(
            ALGCELL_CTX_HEAMO_CTXID(ctx), result.curve_rbc, result.curve_plt, AlgSamp_ResultCallback, (void*)&result.heamo, result.alarm_results);
        if (ret) {
        ...
        //判断机型
        if (g_machine_type == 1) {
            //低端机型,有五分类就是五分类,疟原虫定性
            std::cout << " 993 机型: " << g_machine_type << std::endl;
            auto iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == WBC_KEY_NUM; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == NE_KEY_PERCENTAGE; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == LY_KEY_PERCENTAGE; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == MO_KEY_PERCENTAGE; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == EO_KEY_PERCENTAGE; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == BA_KEY_PERCENTAGE; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == NE_KEY_NUM; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == LY_KEY_NUM; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == MO_KEY_NUM; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == EO_KEY_NUM; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == BA_KEY_NUM; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == RBC_KEY_NUM; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == HB_KEY_VALUE; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == HCT_KEY_VALUE; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == MCV_KEY_VALUE; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == MCH_KEY_VALUE; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }

            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == MCHC_KEY_VALUE; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == RDW_CV_KEY_VALUE; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == RDW_SD_KEY_VALUE; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == PLT_KEY_NUM; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }

            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == PCT_KEY_VALUE; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == MPV_KEY_VALUE; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == "PDW"; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            //疟原虫定性判断
            iter               = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == "PLS"; });
            if(iter != result.heamo.end())
            {
                float         of = atof((iter->value).c_str());
                AlgCellItem_t pla_qualitative;
                ALGCELL_ITEM_NAME(&pla_qualitative) = "pla_qualitative";
                ALGCELL_ITEM_UNIT(&pla_qualitative) = "-";
                pla_qualitative.open_flag           = true;

                std::cout << " 1098 g_pla_threshold: " << g_pla_threshold << std::endl;

                if (of > g_pla_threshold) {
                    ALGCELL_ITEM_VALUE(&pla_qualitative) = "1";
                    result.heamo.push_back(pla_qualitative);
                }
                else {
                    ALGCELL_ITEM_VALUE(&pla_qualitative) = "0";
                    result.heamo.push_back(pla_qualitative);
                }

                std::cout << " 1109 机型: " << g_machine_type << std::endl;
            }
            else if (g_pla_flag && iter == result.heamo.end()) {
                ALGLogError << __FUNCTION__ << "  " << __LINE__ << "开了疟原虫,但是未找到疟原虫检测 ";
                AlgCellItem_t temp_pla;
                temp_pla.name      = std::string("pla_qualitative");
                temp_pla.unit      = std::string("");
                temp_pla.open_flag = true;
                temp_pla.value = std::string("0");
                result.heamo.push_back(temp_pla);

            }
        }
        if (g_machine_type == 2 || g_machine_type == 3){
            // 中端机型  高端机型
            std::cout << " 1119 机型: " << g_machine_type << std::endl;
            auto iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == WBC_KEY_NUM; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            ...
            //网织红 有核红
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == RET_KEY_NUM; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }

            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == RET_KEY_PERCENTAGE; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == NRBC_KEY_NUM; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == NRBC_KEY_PERCENTAGE; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            std::cout << " 1256 机型: " << g_machine_type << std::endl;
            //疟原虫定量信息
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == "Pr#"; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == "Pt#"; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == "Ps#"; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == "Pg#"; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == "Px#"; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == "E-rosetting#"; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            ...
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == "PLS/5m RBC"; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            iter = std::find_if(result.heamo.begin(), result.heamo.end(), [](const AlgCellItem& p) { return p.name == "PLS/8k WBC"; });
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
            }
            // 疟原虫阴性 阳性
            ...
            if (iter != result.heamo.end()) {
                iter->open_flag = true;
                std::cout << " 1295 机型: " << g_machine_type << std::endl;
                float         of = atof((iter->value).c_str());
                std::cout << " 1297 of: " << of << std::endl;
                std::cout << " 1298 机型: " << g_machine_type << std::endl;
                ...
                std::cout << " 1302 g_pla_threshold: " << g_pla_threshold << std::endl;
                ...
                std::cout << " 1314 机型: " << g_machine_type << std::endl;
            }
        }
        std::cout << " 1312 机型: " << g_machine_type << std::endl;
...
extern std::map<std::string, std::vector<ImageRet_t>> g_image_result;
extern std::vector<std::vector<char>>                 g_img_name_vec;
extern std::vector<std::vector<unsigned char>>        g_img_buff_vec;
...
int AlgCell_HeamoGetImgResult(AlgCtxID_t ctx_id, std::vector<ImageRet_t>& result, uint32_t timeout)
{
    ...
    // std::cout << "g_image_result 大小： " << g_image_result.size() << std::endl;
    // std::cout << "g_img_name_vec 大小： " << g_img_name_vec.size() << std::endl;
    // std::cout << "g_img_buff_vec 大小： " << g_img_buff_vec.size() << std::endl;

    for (auto iterm : g_image_result) {
        for (int i =0; i< iterm.second.size();i++ ) {
          result.emplace_back(iterm.second.at(i));
          ALGLogInfo << "g_image_result 1311 image name : " << iterm.second.at(i).image_name.data() ;
        }
        ALGLogInfo << "g_image_result 975 image name : " << iterm.first << " image count : " << iterm.second.size();
    }
    ALGLogInfo << "返回图片结果 result.size() : " << result.size() ;
...
std::map<std::string, std::vector<ImageRet_t>> AlgCell_HeamoGetImgResultMap(AlgCtxID_t ctx_id, uint32_t timeout)
{
    ...
    return std::move(g_image_result);
}
...
int AlgCell_HeamoGetMicFluResult(AlgCtxID_t ctx_id, MicFluInfo_t& result, uint32_t timeout){
    ...
    if (ALGCELL_CTX_FUNC_MASK(ctx) & ALGCELL_FUNC_HEAMO) {
      ...
      ret = HeamoGetMicFluResult(ALGCELL_CTX_HEAMO_CTXID(ctx), result);
      if (ret) {
          ALGLogError << "Failed to get heamo result, err = " << ret;
          return ret;
      }
...
int AlgCell_HeamoClose(AlgCtxID_t ctx_id) {
  ...
  g_image_result.clear();
  g_img_name_vec.clear();
  g_img_buff_vec.clear();
  return 0;
}
...
int AlgCell_ClarityPushImage(AlgCtxID_t ctx_id, uint32_t group_idx, uint32_t chl_idx, AlgCellImg_t *img_array,
  ...
  // std::cout << "1090 AlgCell_ClarityPushImage 图片大小 高: " << (&img_array[0])->height << "  宽 " << (&img_array[0])->width << std::endl;
  auto complementary_params_iter = complementary_params.find(VIEW_PAIR_IDX);
  if (complementary_params_iter != complementary_params.end()) {
    const int &view_pair_idx = (int) complementary_params_iter->second;
...
int AlgCell_ClarityGetResultAll(AlgCtxID_t ctx_id, std::vector<AlgClarityValue_t> &list) {
  ...
  if (result) {
      ALGLogError << "图片任务队列处理未完成 result: " << result;
      return -2;
  }
...
int AlgCell_ClarityGetResultBest(AlgCtxID_t ctx_id, uint32_t* index, float* value, IMAGE_TYPE& type)
{
    ...
    auto ret = Clarity_WaitCplt(ALGCELL_CTX_CLARITY_CTXID(ctx), 0xFFFFFF);
    // auto end = duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count();
    // auto cost_time = end - start;
    // ALGLogInfo << "processing time: " << cost_time;
    if (ret != 0) {
        ALGLogError << "Clarity_WaitCplt err : timeOut " << ret << " : " << 0xFFFFFF;
        EVERROR(EVID_ERR, "Clarity_WaitCplt  err")
        return -2;
    }
    int temp_type=-1;
    int best_value = Clarity_GetBestValue(ALGCELL_CTX_CLARITY_CTXID(ctx), index, value, temp_type);
    type     = static_cast<IMAGE_TYPE> (temp_type);
    ALGLogInfo << " 1190 AlgCell_ClarityGetResultBest, 返回值： " << best_value;

    return best_value;
}

int AlgCell_ClarityGetResultFarNear(AlgCtxID_t ctx_id, uint32_t* index, float* value, IMAGE_TYPE& type)
{
    return AlgCell_ClarityGetResultBest(ctx_id, index, value,type);
}

int AlgCell_ClarityGetResultCoarse(AlgCtxID_t ctx_id, uint32_t* index, float* value, IMAGE_TYPE& type)
{
    return AlgCell_ClarityGetResultBest(ctx_id, index, value, type);
}
