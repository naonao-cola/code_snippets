                          ...
                          const float&              PV5_value)
{
    ...
    std::string WBC_value_s, NEUT_percentage_s, LYMPH_percentage_s, MONO_percentage_s, EOS_percentage_s, BASO_percentage_s, NEUT_value_s,
        ...
        RDW_CV_value_s, RDW_SD_value_s, NRBC_value_s, PLT_value_s, MPV_value_s, PCT_value_s, RET_value_s, RET_percentage_s, PV_value_s, PV1_value_s,
        PV2_value_s, PV3_value_s, PV4_value_s, PV5_value_s;
    // 数据截断
    CutFloatToString(WBC_value, 2, WBC_value_s);
    CutFloatToString(NEUT_percentage, 2, NEUT_percentage_s);
    CutFloatToString(LYMPH_percentage, 2, LYMPH_percentage_s);
    CutFloatToString(MONO_percentage, 2, MONO_percentage_s);
    CutFloatToString(EOS_percentage, 2, EOS_percentage_s);

    CutFloatToString(BASO_percentage, 2, BASO_percentage_s);
    CutFloatToString(NEUT_value, 2, NEUT_value_s);
    CutFloatToString(LYMPH_value, 2, LYMPH_value_s);
    CutFloatToString(MONO_value, 2, MONO_value_s);
    CutFloatToString(EOS_value, 2, EOS_value_s);

    CutFloatToString(BASO_value, 2, BASO_value_s);
    CutFloatToString(RBC_value, 2, RBC_value_s);
    CutFloatToString(HGB_value, 2, HGB_value_s);
    CutFloatToString(HCT_value, 2, HCT_value_s);
    CutFloatToString(MCV_value, 2, MCV_value_s);

    CutFloatToString(MCH_value, 2, MCH_value_s);
    CutFloatToString(MCHC_value, 2, MCHC_value_s);
    ...
    CutFloatToString(PLT_value, 2, PLT_value_s);
    CutFloatToString(MPV_value, 2, MPV_value_s);
    CutFloatToString(PCT_value, 2, PCT_value_s);
    CutFloatToString(RET_value, 3, RET_value_s);
    CutFloatToString(RET_percentage, 2, RET_percentage_s);

    // 疟原虫
    CutFloatToString(PV_value, 3, PV_value_s);
    CutFloatToString(PV1_value, 3, PV1_value_s);
    CutFloatToString(PV2_value, 3, PV2_value_s);
    CutFloatToString(Pv3_value, 3, PV3_value_s);
    CutFloatToString(PV4_value, 3, PV4_value_s);
    CutFloatToString(PV5_value, 3, PV5_value_s);
    ...
    if (qc) {
        ...
        if (g_pla_flag) {
            HEAMO_RESULT(callback, userdata, "PLA", "/uL", PV_value_s.c_str())
            HEAMO_RESULT(callback, userdata, "PV1", "/uL", PV1_value_s.c_str())
            HEAMO_RESULT(callback, userdata, "PV2", "/uL", PV2_value_s.c_str())
            HEAMO_RESULT(callback, userdata, "PV3", "/uL", PV3_value_s.c_str())
            HEAMO_RESULT(callback, userdata, "PV4", "/uL", PV4_value_s.c_str())
            HEAMO_RESULT(callback, userdata, "PV5", "/uL", PV5_value_s.c_str())
        }

        if (required_ret) {
            HEAMO_RESULT(callback, userdata, RET_KEY_NUM, "10^9/L", "-")
        ...
        if (required_ret) {
            HEAMO_RESULT(callback, userdata, RET_KEY_NUM, "10^9/L", RET_value_s.c_str())
...
int TempHeamoWriteResult(const HeamoCnt_t& cnt)
{
    ...
    if (max_img_nums == 0) {
        ALGLogInfo << "Heamo csv file will not be created cause zero germ imgs accepted.";
        return 0;
    }
                         ...
                         std::vector<std::string>& alarm_str_v)
{
    ...
    float RET_value = (float)(cnt->RET * dilution_ratio_ret / (rbc_channel_nums * calib_volume_ret + ADD_DENOMINATOR));
    ...
    float PV5_VALUE = 0;
    if (g_pla_flag) {
        PV_VALUE  = (float)(cnt->PLA * dilution_ratio_rbc / (rbc_channel_nums * calib_volume_rbc + ADD_DENOMINATOR));
        PV1_VALUE = (float)(cnt->PV1 * dilution_ratio_rbc / (rbc_channel_nums * calib_volume_rbc + ADD_DENOMINATOR));
        PV2_VALUE = (float)(cnt->PV2 * dilution_ratio_rbc / (rbc_channel_nums * calib_volume_rbc + ADD_DENOMINATOR));
        PV3_VALUE = (float)(cnt->PV3 * dilution_ratio_rbc / (rbc_channel_nums * calib_volume_rbc + ADD_DENOMINATOR));
        PV4_VALUE = (float)(cnt->PV4 * dilution_ratio_rbc / (rbc_channel_nums * calib_volume_rbc + ADD_DENOMINATOR));
        PV5_VALUE = (float)(cnt->PV5 * dilution_ratio_rbc / (rbc_channel_nums * calib_volume_rbc + ADD_DENOMINATOR));
    }
    PV_VALUE = PV1_VALUE + PV2_VALUE + PV3_VALUE + PV4_VALUE;
    ...
    float NEUT_value  = (float)(cnt->NEU * dilution_ratio_wbc / (wbc_channel_nums * calib_volume_wbc + ADD_DENOMINATOR));
...
std::vector<ResultTypeMap_t> heamo_result_map = {
    ...
    MODIFY_TYPE_MAP_DEF(ModifyTypeMap_t, BA_KEY_NUM, "%", HEAMO_RESULT_QC_ERASE, 0, HEAMO_RESULT_REQUIRED_ALL, 2, HEAMO_MODIFY_TYPE_WBC),
    ...
    MODIFY_TYPE_MAP_DEF(ModifyTypeMap_t, RET_KEY_NUM, "10^9/L", HEAMO_RESULT_QC_ERASE, 0, HEAMO_RESULT_REQUIRED_RET, 2, HEAMO_MODIFY_TYPE_RBC),
    ...
    MODIFY_TYPE_MAP_DEF(ModifyTypeMap_t, "PLA", "/uL", HEAMO_RESULT_QC_KEEP, 0, HEAMO_RESULT_REQUIRED_ALL, 2, HEAMO_MODIFY_TYPE_RBC),
    MODIFY_TYPE_MAP_DEF(ModifyTypeMap_t, "PV1", "/uL", HEAMO_RESULT_QC_KEEP, 0, HEAMO_RESULT_REQUIRED_ALL, 2, HEAMO_MODIFY_TYPE_RBC),
    MODIFY_TYPE_MAP_DEF(ModifyTypeMap_t, "PV2", "/uL", HEAMO_RESULT_QC_KEEP, 0, HEAMO_RESULT_REQUIRED_ALL, 2, HEAMO_MODIFY_TYPE_RBC),
    MODIFY_TYPE_MAP_DEF(ModifyTypeMap_t, "PV3", "/uL", HEAMO_RESULT_QC_KEEP, 0, HEAMO_RESULT_REQUIRED_ALL, 2, HEAMO_MODIFY_TYPE_RBC),
    MODIFY_TYPE_MAP_DEF(ModifyTypeMap_t, "PV4", "/uL", HEAMO_RESULT_QC_KEEP, 0, HEAMO_RESULT_REQUIRED_ALL, 2, HEAMO_MODIFY_TYPE_RBC),
    MODIFY_TYPE_MAP_DEF(ModifyTypeMap_t, "PV5", "/uL", HEAMO_RESULT_QC_KEEP, 0, HEAMO_RESULT_REQUIRED_ALL, 2, HEAMO_MODIFY_TYPE_RBC),
};
                            ...
                            std::vector<std::string>& alarm_str_v)
{
    ...
    float WBC_value, NEUT_value, LYMPH_value, MONO_value, EOS_value, BASO_value, NRBC_value, IG_value, wbc_agglomeration, RDW_CV_value, RDW_SD_value,
        MCV_value, RBC_value, HGB_value, MCHC_value, rbc_agglomeration, PLT_value, plt_agglomeration, RET_percentage;
