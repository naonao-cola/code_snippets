...
void AlgoA::getParam(InferTaskPtr task)
{
    ...
    // ------调节公差参数 2024/12/27 -------//
    error_index_ = param_json["param"]["error_adjust_index"];
    error_second_ = param_json["param"]["error_second"];
    // 防空
    // 开口模块序号
    if (error_index_.size() != product_rows_ * product_cols_) {
        error_index_list.resize(product_rows_ * product_cols_, 0);
    }
    else {
        error_index_list = error_index_.get<std::vector<int>>();
        //for (int i = 0; i < error_index_.size(); i++){
        //    error_index_list.push_back(error_index_[i]);
        //}
    }
    // 开口测量项个数
    if (error_second_.size() != 11) {
        error_second_list.resize(11, 0);
    }
    else {
        error_second_list = error_second_.get<std::vector<double>>();
        //for (int i = 0; i < error_second_.size(); i++) {
        //    error_second_list.push_back(error_second_[i]);
        //}
    }
    // ----------------------------------------//
...
void AlgoA::CropTaskImg(const cv::Mat& input_img)
{
    ...
    for (int m = 0; m < retFinalNum; m++) {
        ...
        if (enable_affineImg_) {
            cropImgP = PerspectTransform(dis, pt_vec, box, invertMat);
    ...
    for (int i = 0; i < lc_vec.size(); i++) {
        ...
        if (enable_saveNGImg_) {
            ...
            if (!lc_vec[i].classify_status) {               
                std::string saveNGPath = NGSavePath_ + "classify/NG/" + imgName_ + "_" + std::to_string(i) + ".jpg";
                cv::imwrite(saveNGPath, lc_vec[i].img);
            }
            else{
                std::string saveNGPath = NGSavePath_ + "classify/OK/" + imgName_ + "_" + std::to_string(i) + ".jpg";
                cv::imwrite(saveNGPath, lc_vec[i].img);
            }
            if (!lc_vec[i].segment_status) {              
                std::string saveNGPath = NGSavePath_ + "segment/NG/" + imgName_ + "_" + std::to_string(i) + ".jpg";
                cv::imwrite(saveNGPath, lc_vec[i].img);
            }
            else{
                std::string saveNGPath = NGSavePath_ + "segment/OK/" + imgName_ + "_" + std::to_string(i) + ".jpg";
                cv::imwrite(saveNGPath, lc_vec[i].img);
            }
        }
        ...
        // ---------调节公差 2024/12/27---//
        if (error_index_list[lc_vec[i].index] == 1) {
            error_a_  = error_second_list[0];
            error_b1_ = error_second_list[1];
            error_b2_ = error_second_list[2];
            error_b3_ = error_second_list[3];
            error_c1_ = error_second_list[4];
            error_c2_ = error_second_list[5];
            error_d1_ = error_second_list[6];
            error_d2_ = error_second_list[7];
            error_e_  = error_second_list[8];
            error_p_  = error_second_list[9];
            error_f_  = error_second_list[10];
        }
        //-------------------------------//
