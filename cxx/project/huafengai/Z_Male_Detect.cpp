...
bool Z_Male_Detect::get_param(InferTaskPtr task, std::vector<AlgoResultPtr> pre_results)
{
    ...
    window_h_      = Tival::JsonHelper::GetParam(task_param_json["param"], "window_h", 400);
    window_w_  = Tival::JsonHelper::GetParam(task_param_json["param"], "window_w", 400);
    overlap_ratio_ = Tival::JsonHelper::GetParam(task_param_json["param"], "overlap_ratio", 0.5);
    ai_open_       = Tival::JsonHelper::GetParam(task_param_json["param"], "ai_open", false);
    batch_num_     = Tival::JsonHelper::GetParam(task_param_json["param"], "batch_num", 12);
    segthreshold_  = Tival::JsonHelper::GetParam(task_param_json["param"], "segthreshold", 0.5);
    ...
    if (!tempImagePoint_1_.empty()) {
        ...
        else if (z_male_x_num_ == 13 && z_male_y_num_ == 8) {
            template_mat_1_ = cv::Mat::zeros(z_male_y_num_, z_male_x_num_, CV_32FC(6));
            template_mat_1_ = ret(cv::Range(0, z_male_y_num_), cv::Range(0, z_male_x_num_)).clone();
        }
        else if (z_male_x_num_ == 13 && z_male_y_num_==6) {
            template_mat_1_ = cv::Mat::zeros(z_male_y_num_, z_male_x_num_, CV_32FC(6));
            template_mat_1_ = ret(cv::Range(0, z_male_y_num_), cv::Range(0, z_male_x_num_)).clone();
...
cv::Mat Z_Male_Detect::get_col_row(std::vector<cv::Point2f> basis_info_vec)
{
    ...
    if (z_male_x_num_ == 13 && z_male_y_num_==8) {
        ret = cv::Mat::zeros(rank.size(), 13, CV_32FC(6));
        // 将预留的位置补黑,这几个位置pw的产品是空的，如果非空，下面的操作补全即可
        ret.at<cv::Vec6f>(1, 12)[0] = -1000;
        ret.at<cv::Vec6f>(1, 12)[1] = -1000;
        ret.at<cv::Vec6f>(1, 12)[3] = -1000;
        ret.at<cv::Vec6f>(3, 12)[0] = -1000;
        ret.at<cv::Vec6f>(3, 12)[1] = -1000;
        ret.at<cv::Vec6f>(3, 12)[3] = -1000;
        ret.at<cv::Vec6f>(5, 12)[0] = -1000;
        ret.at<cv::Vec6f>(5, 12)[1] = -1000;
        ret.at<cv::Vec6f>(5, 12)[3] = -1000;
        ret.at<cv::Vec6f>(7, 12)[0] = -1000;
        ret.at<cv::Vec6f>(7, 12)[1] = -1000;
        ret.at<cv::Vec6f>(7, 12)[3] = -1000;
    }
    if (z_male_x_num_ == 13 && z_male_y_num_ == 6) {
        ret = cv::Mat::zeros(rank.size(), 13, CV_32FC(6));
        // 将预留的位置补黑,这几个位置pw的产品是空的，如果非空，下面的操作补全即可
        ret.at<cv::Vec6f>(1, 12)[0] = -1000;
        ret.at<cv::Vec6f>(1, 12)[1] = -1000;
...
std::vector<cv::Point2f> Z_Male_Detect::img_process_1(const cv::Mat& src, const cv::Mat& gray_img, AlgoResultPtr algo_result, double& angle) noexcept
{
    ...
    if (ai_open_) {
        cv::Rect img_rect(detect_left_x_, detect_left_y_, detect_width_, detect_height_);
        cv::Mat tmp_mask = src(img_rect).clone();
        std::vector<cv::Rect> windows_rect;
        std::vector<cv::Mat> windows_img;
        crop_img(tmp_mask, img_rect, windows_rect, windows_img);
        total_center = cal_img(gray_img,img_rect, windows_rect, windows_img);
        return total_center;
    ...
    //cv::threshold(gamma_img, th_img, thre_value, 255, cv::THRESH_BINARY);
    // 形态学处理
    cv::Mat elementX = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(7, 1));
    ;   // X方向腐蚀
    ...
    for (int i = 0; i < center_contours.size(); ++i) {
        ...
        if (!std::isnan(ret_center.x)) {
            algo_result->result_info.push_back({{"label", "fuzhu"},
                                                {"shapeType", "polygon"},
...
void Z_Male_Detect::cal_data(const cv::Mat& src, const cv::Mat& gray_img, std::vector<cv::Point2f> pt_vec_1, std::vector<cv::Point2f> pt_vec_2, double angle, AlgoResultPtr algo_result) noexcept
{
    ...
    for (int i = 0; i < template_mat_1_.rows; i++) {
        for (int j = 0; j < template_mat_1_.cols; j++) {
            ...
            algo_result->result_info.push_back({{"label", "fuzhu"},
                                                {"shapeType", "polygon"},
                                                {"points",
                                                 {{org_pt.x - error_pix, org_pt.y - error_pix},
                                                  {org_pt.x + error_pix, org_pt.y - error_pix},
                                                  {org_pt.x + error_pix, org_pt.y + error_pix},
                                                  {org_pt.x - error_pix, org_pt.y + error_pix}}},
                                                {"result", pixel[3]}});
            if (pixel[3] == 0) {
                algo_result->result_info.push_back({{"label", "J"},
                                                    {"shapeType", "point"},
                                                    ...
                                                    {"result",
                                                     {{"is_ok", false},
                                                      ...
                                                      {"measured_y", 0},
                                                      {"TP", 0},
                                                      {"index", i * template_mat_2_.rows + j}}}});
            }
        }
    }
    for (int i = 0; i < template_mat_2_.rows; i++) {
        for (int j = 0; j < template_mat_2_.cols; j++) {
            ...
            if (pixel[3] == 0) {
                algo_result->result_info.push_back({{"label", "T"},
                                                    {"shapeType", "point"},
                                                    {"points", {{org_pt.x, org_pt.y}}},
                                                    {"result",
                                                     {{"is_ok", false},
    ...
    // 误差校正
    for (int i = 0; i < pt_info_vec_1.size(); i++) {
        if (pt_info_vec_1[i].index < 0 || (pt_info_vec_1[i].org_pt.x < -10 && pt_info_vec_1[i].org_pt.y < -10))
            continue;
        ...
        algo_result->result_info.push_back({{"label", "J"},
                                            ...
                                            {"result",
                                             {{"is_ok", pt_info_vec_1[i].is_ok},
                                              {"x_off", pt_info_vec_1[i].offset_x},
                                              {"y_off", pt_info_vec_1[i].offset_y},
                                              {"std_x", pt_info_vec_1[i].org_pt.x},
                                              {"std_y", pt_info_vec_1[i].org_pt.y},
                                              {"measured_x", pt_info_vec_1[i].cal_pt.x},
                                              {"measured_y", pt_info_vec_1[i].cal_pt.y},
                                              {"TP", 2 * sqrt(pt_info_vec_1[i].offset_x * pt_info_vec_1[i].offset_x + pt_info_vec_1[i].offset_y * pt_info_vec_1[i].offset_y)},
                                              {"index", pt_info_vec_1[i].index}}}});
    }
    for (int i = 0; i < pt_info_vec_2.size(); i++) {
        ...
        algo_result->result_info.push_back({{"label", "T"},
                                            ...
                                            {"result",
                                             {{"is_ok", pt_info_vec_2[i].is_ok},
                                              {"x_off", pt_info_vec_2[i].offset_x},
                                              {"y_off", pt_info_vec_2[i].offset_y},
                                              {"std_x", pt_info_vec_2[i].org_pt.x},
                                              {"std_y", pt_info_vec_2[i].org_pt.y},
                                              {"measured_x", pt_info_vec_2[i].cal_pt.x},
                                              {"measured_y", pt_info_vec_2[i].cal_pt.y},
                                              {"TP", 2 * sqrt(pt_info_vec_2[i].offset_x * pt_info_vec_2[i].offset_x + pt_info_vec_2[i].offset_y * pt_info_vec_2[i].offset_y)},
                                              {"index", pt_info_vec_2[i].index}}}});
    }
    if (!status_flag) {
        algo_result->judge_result = 0;
    }
...
void Z_Male_Detect::crop_img(cv::Mat img, cv::Rect img_rect, std::vector<cv::Rect>& windows_rect, std::vector<cv::Mat>& windows_img)
{
    ...
    while (x < img.cols) {
        ...
        while (y < img.rows) {
            ...
            y += step_y;
            if (y + window_h_> img.rows){
                break;
            }
        }
        x += step_x;
        if (x+ window_w_>img.cols){
            break;
        }
    }
}

std::vector<cv::Point2f> Z_Male_Detect::cal_img(cv::Mat org_img, cv::Rect img_rect, std::vector<cv::Rect> windows_rect, std::vector<cv::Mat> windows_img)
{
    cv::Mat                  dis;
    cv::cvtColor(org_img, dis, cv::COLOR_GRAY2BGR);
    std::vector<cv::Point2f> pts;
    int loopNum = std::ceil(float(windows_img.size()) / batch_num_);
    AIRuntimeInterface*      ai_obj  = GetAIRuntime();

    for (int loop = 0; loop < loopNum; loop++) {
        int start = loop * batch_num_;
        int end   = (std::min)(static_cast<int>(windows_img.size()), start + batch_num_);
        std::vector<cv::Mat> cropImgList;
        std::vector<cv::Rect> cropRectList;
        for (int k = start; k < end; k++) {
            cropImgList.push_back(windows_img[k]);
            cropRectList.push_back(windows_rect[k]);
        }
        ...
        for (int i = 0; i < rst->itemList.size(); i++){
            for (auto& box : rst->itemList[i]){
                if (box.confidence <= segthreshold_) {
                    LOGD("filter by confidence {}", box.confidence);
                    continue;
                }
                std::vector<cv::Point> tempMask = box.mask[0];
                cv::Rect               maskRect = cv::boundingRect(tempMask);
                float                  maskArea = maskRect.width * maskRect.height;
                double                 area     = cv::contourArea(tempMask);
                if (area < area_range_b_)
                    continue;
                if (maskRect.height <= rect_height_t_ && maskRect.height >= rect_height_b_ && maskRect.width >= rect_width_b_ && maskRect.width <= rect_width_t_) {
                    continue;
                }
                double center_x = maskRect.x + maskRect.width / 2  + cropRectList[i].x + img_rect.x;
                double center_y = maskRect.y + maskRect.height / 2 + cropRectList[i].y + img_rect.y;
    ...
    for (int i = 0; i < pts.size();i++) {
        ...
        for (int j = i+1; j < pts.size();j++) {
            if (index[j] > 0)
                continue;
            double dis = connector::dist_p2p(pts[i], pts[j]);
            if (dis<40){
                sum_x += pts[j].x;
                sum_y += pts[j].y;
                count++;
        ...
        cv::circle(dis, cv::Point2f(sum_x, sum_y), 1, cv::Scalar(0, 0, 255), 2);
    }
