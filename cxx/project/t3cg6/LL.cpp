/**
 * @FilePath     : /t3cg6/src/project/RL.cpp
 * @Description  :
 * @Author       : naonao
 * @Date         : 2024-12-03 15:53:05
 * @Version      : 0.0.1
 * @LastEditors  : naonao
...
AlgoResultPtr LL::RunAlgo(InferTaskPtr task, std::vector<AlgoResultPtr> pre_results)
{
    LOGI("LL start run! exec update 2024/12/25");
    ...
    LOGI("LL run finished!");
    return algo_result;
}

void LL::getParam(InferTaskPtr task)
{
...
std::vector<cv::Rect> LL::img_process(cv::Mat src, InferTaskPtr task, AlgoResultPtr algo_result)
{
    ...
    // 查找边界
    xx::get_edge(src, th_low_, th_high_, blur_w_, blur_h_, row_beg, row_end, col_beg, col_end, continuous_, th_value_, 3);
