/**
 * @FilePath     : /t3cg6/src/project/LS.cpp
 * @Description  :
 * @Author       : naonao
 * @Date         : 2024-12-03 15:52:10
 * @Version      : 0.0.1
 * @LastEditors  : naonao
 * @LastEditTime : 2024-12-03 16:44:38
...
AlgoResultPtr RS::RunAlgo(InferTaskPtr task, std::vector<AlgoResultPtr> pre_results)
{
    LOGI("RS start run! exec update 2024/12/25");
    ...
    LOGI("RS run finished!");
    return algo_result;
}

void RS::getParam(InferTaskPtr task)
{
 ...
 std::vector<cv::Rect> RS::img_process(cv::Mat src, InferTaskPtr task, AlgoResultPtr algo_result)
{
    ...
    //xx::Timer t;
    //查找边界
    xx::get_edge(src, th_low_, th_high_, blur_w_, blur_h_, row_beg, row_end, col_beg, col_end, continuous_, th_value_, 2);
    //t.out();
    ...
    for (int i = 0; i < feature.size();i++) {
        ...
        //排除最下面的干扰,右短边增加一个
        bool c = (std::abs(tmp_rect.y + tmp_rect.height / 2 - row_end) < 50) || (std::abs(tmp_rect.y + tmp_rect.height - row_end) < 40) || (std::abs(tmp_rect.y - row_beg) < 70);
        if (a && (b || c)) {
            continue;
        }
