...
void detector_postprocess(const Mat& pred_map,
                          BoxArray&  boxes,
                          int        src_h,
                          int        src_w,
                          int        dst_h,
...
public:
    ...
    virtual bool startup(const string& engine_file,
                         int           gpuid,
                         float         mask_thresh,
                         float         box_thresh,
                         float         unclip_ratio,
    ...
    virtual bool preprocess(Job& job, const Mat& image) override
    {
        ...
        CUDAKernel::resize_bilinear_and_normalize(
            image_device, image.cols * 3, image.cols, image.rows, tensor->gpu<float>(), input_width_, input_height_, normalize_, preprocess_stream);
    ...
    {
        box_thresh_ = confidence_threshold;
    }
...
shared_ptr<TextDetector> create_detector(const string& engine_file,
                                         int           gpuid,
                                         float         mask_thresh,
                                         float         box_thresh,
                                         float         unclip_ratio,
                                         ...
                                         bool          use_multi_preprocess_stream)
{
    ...
    if (!instance->startup(engine_file, gpuid, mask_thresh, box_thresh, unclip_ratio, min_size, max_candidates, use_multi_preprocess_stream)) {
        instance.reset();
    }
    return instance;
}
...
void image_to_tensor(const cv::Mat& image, shared_ptr<TRT::Tensor>& tensor, int ibatch)
{
    ...
    CUDAKernel::resize_bilinear_and_normalize(
        image_device, image.cols * 3, image.cols, image.rows, tensor->gpu<float>(ibatch), input_size.width, input_size.height, normalize, stream);
    tensor->synchronize();
}
};   // namespace DBDetector
};   // namespace PaddleOCR