...
cv::Mat get_rotate_crop_image(const cv::Mat& src_image, const vector<vector<int>>& box)
{
    ...
    auto [minPoint, maxPoint] = std::minmax_element(points.begin(), points.end(), [](const cv::Point& a, const cv::Point& b) { return a.x < b.x; });
    int left                  = minPoint->x;
    int right                 = maxPoint->x;
    minPoint                  = std::min_element(points.begin(), points.end(), [](const cv::Point& a, const cv::Point& b) { return a.y < b.y; });
    maxPoint                  = std::max_element(points.begin(), points.end(), [](const cv::Point& a, const cv::Point& b) { return a.y < b.y; });
    int top                   = minPoint->y;
    ...
    std::array<cv::Point2f, 4> srcPoints = {cv::Point2f(points[0].x - left, points[0].y - top),
    ...
    std::array<cv::Point2f, 4> dstPoints = {cv::Point2f(0.0f, 0.0f), cv::Point2f(width, 0.0f), cv::Point2f(width, height), cv::Point2f(0.0f, height)};
...
void draw_ocr_box_txt(cv::Mat& image, DBDetector::BoxArray& boxes, vector<string>& texts, string save_path)
{
    ...
    cuosd_apply(
        context, gpu_image->data0, gpu_image->data1, gpu_image->width, gpu_image->stride, gpu_image->height, cuOSDImageFormat::RGB, stream, true);
