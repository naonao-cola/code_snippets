...
#pragma omp parallel for num_threads(infers_.size())
        for (int i = 0; i < gpuids.size(); ++i) {
            auto& gpuid = gpuids[i];
            infers_[i]  = Yolo::create_infer(engine_file, type, gpuid, confidence_threshold, nms_threshold, nms_method, max_objects);
        }
...
public:
    ...
    {
        infers_[get_gpu_index()]->setConfidence(confidence_threshold);
    }
    virtual void setNMS(float nms_threshold) override
    {

        infers_[get_gpu_index()]->setNMS(nms_threshold);
    }
...
shared_ptr<MultiGPUInfer> create_multi_gpu_infer(const string&     engine_file,
                                                 Type              type,
                                                 const vector<int> gpuids,
                                                 float             confidence_threshold,
                                                 float             nms_threshold,
                                                 ...
                                                 int               max_objects)
{
    ...
    if (!impl->startup(engine_file, type, gpuids, confidence_threshold, nms_threshold, nms_method, max_objects)) {
        instance.reset();
    }
    return instance;
}
