...
static float probiou(const Box& obb1, const Box& obb2, float eps = 1e-7)
{
    ...
    float t3 = std::log(((a1 + a2) * (b1 + b2) - std::pow(c1 + c2, 2)) /
                            (4 * std::sqrt(std::max(a1 * b1 - c1 * c1, 0.0f)) * std::sqrt(std::max(a2 * b2 - c2 * c2, 0.0f)) + eps) +
                        eps);
...
static BoxArray cpu_nms(BoxArray& boxes, float threshold)
{
    ...
    std::sort(boxes.begin(), boxes.end(), [](BoxArray::const_reference a, BoxArray::const_reference b) { return a.confidence > b.confidence; });
...
public:
    ...
    virtual bool startup(const string& file,
                         int           gpuid,
                         float         confidence_threshold,
                         float         nms_threshold,
                         ...
                         bool          use_multi_preprocess_stream)
    {
        ...
        iLogger::set_logger_save_directory("d:/logs/yolov8");
    ...
    virtual void worker(promise<bool>& result) override
    {
        ...
        while (get_jobs_and_wait(fetch_jobs, max_batch_size)) {
            ...
            for (int ibatch = 0; ibatch < infer_batch_size; ++ibatch) {
                ...
                decode_kernel_invoker(image_based_output,
                                      output->size(1),
                                      num_classes,
            ...
            for (int ibatch = 0; ibatch < infer_batch_size; ++ibatch) {
                ...
                for (int i = 0; i < count; ++i) {
                    float* pbox = parray + 1 + i * NUM_BOX_ELEMENT;   // cx, cy, w, h, angle, confidence,
                                                                      // class_label, keepflag
                    int label    = pbox[6];
                    int keepflag = pbox[7];
                    if (keepflag == 1) {
    ...
    virtual bool preprocess(Job& job, const Mat& image) override
    {
        ...
        checkCudaRuntime(
            cudaMemcpyAsync(affine_matrix_device, affine_matrix_host, sizeof(job.additional.d2i), cudaMemcpyHostToDevice, preprocess_stream));

        CUDAKernel::warp_affine_bilinear_and_normalize_plane(image_device,
                                                             image.cols * 3,
                                                             image.cols,
                                                             image.rows,
                                                             tensor->gpu<float>(),
    ...
    virtual void setConfidence(float confidence_threshold) override
    {
        confidence_threshold_ = confidence_threshold;
    }
    virtual void setNMS(float nms_threshold) override
    {
        nms_threshold_ = nms_threshold;
    }
