...
{
    if (e != CUDA_SUCCESS) {
        // Map CUresult to cudaError_t for error message handling
        // Since cuGetErrorString/cuGetErrorName require cuda.lib which may not be available,
        // we use the runtime error string instead with a generic message
        INFOE("CUDA Driver error %s # Error code = %d in file %s:%d", call, e, file, line);
        return false;
    }
    return true;
}
...
std::string device_description()
{
    ...
    return iLogger::format("[ID %d]<%s>[arch %d.%d][GMEM %.2f GB/%.2f GB]",
                           device_id,
                           prop.name,
                           prop.major,
                           prop.minor,
