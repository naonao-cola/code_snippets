...
void infer(std::string engine_path, std::string img_path, std::string prompt)
{
    ...
    std::stable_sort(h_keep_idx.begin(), h_keep_idx.end(), [&](int a, int b) {
        return h_logits[a] > h_logits[b];   // 按得分降序排序
    });
    std::cout << "h_masks.size() " << h_masks.size() << std::endl;
    ...
    for (size_t i = 0; i < h_keep_idx.size(); ++i) {
        ...
        std::cout << "mask: " << c_mask_mat << std::endl;
        ...
        c_mask_mat.convertTo(c_mask_mat, CV_8UC1);
        if (cv::countNonZero(c_mask_mat) == 0) {
            std::cout << "No mask found" << std::endl;
            continue;
        };
        ...
        std::string filename = "/home/greatek/wangww/demo/cxx/sam_test/" + std::to_string(i) + ".jpg";

        cv::imwrite(filename, c_mask_mat);
    }
    cudaFree(dImage);
    cudaFree(dIds);
    cudaFree(dAttn);
