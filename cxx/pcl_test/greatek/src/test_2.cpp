/**
 * @FilePath     : /pcl_test/src/test_2.cpp
 * @Description  :
 * @Author       : weiwei.wang
 * @Date         : 2026-01-20 13:49:49
 * @Version      : 0.0.1
 * @LastEditors  : weiwei.wang
 * @LastEditTime : 2026-01-22 10:15:42
        ...
        if (imgColorReg.width > 0 && imgDepthReg.width > 0) {
            ...
            for (size_t i = 0; i < result.boxes.size(); ++i) {
                if (i < result.class_ids.size() && (result.class_ids[i] == 0 || result.class_ids[i] == 39) && i < result.masks.size()) {   // 假设 0 是 person 类 或 39 是 杯子类
                    ...
                    cv::Mat              locations;   // 使用findNonZero加速
                    ...
                    if (!objPixels.empty()) {
                        ...
                        if (!person_points.empty()) {
                            ...
                            pcl::StatisticalOutlierRemoval<pcl::PointXYZRGB> sor;   // 统计滤波
                            ...
                            if (!sor_cloud->empty()) {   // 质心计算
                                double sum_x = 0, sum_y = 0, sum_z = 0;
                                for (const auto& pt : sor_cloud->points) {
                                    sum_x += pt.x;
                                    sum_y += pt.y;
                                    sum_z += pt.z;
                                }
                                float avg_x = sum_x / sor_cloud->size();
                                float avg_y = sum_y / sor_cloud->size();
                                float avg_z = sum_z / sor_cloud->size();

                                // Display on image
                                std::string text     = cv::format("(%.2f, %.2f, %.2f)", avg_x, avg_y, avg_z);
                                cv::Point   text_pos = result.boxes[i].tl();
                                text_pos.y -= 10;
                                if (text_pos.y < 0)
                                    text_pos.y = 15;
                                cv::putText(color_img, text, text_pos, cv::FONT_HERSHEY_SIMPLEX, 1.2, cv::Scalar(0, 0, 255), 2);
                            }
                        }
