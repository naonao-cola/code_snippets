...
/////////////////////////////////////////////////////////////////////////
void convert_nv12_to_bgr_invoke(const uint8_t* y, const uint8_t* uv, int width, int height, int linesize, uint8_t* dst, cudaStream_t stream)
{
    ...
    checkCudaKernel(convert_nv12_to_bgr_kernel<<<grid, block, 0, stream>>>(y, uv, width, height, linesize, dst, total));
}
                                              ...
                                              cudaStream_t stream)
{
    ...
    checkCudaKernel(warp_affine_bilinear_and_normalize_plane_kernel<<<grid, block, 0, stream>>>(
        src, src_line_size, src_width, src_height, dst, dst_width, dst_height, const_value, matrix_2_3, norm, jobs));
}


void warp_affine_bilinear_and_normalize_focus(uint8_t*     src,
                                              int          src_line_size,
                                              int          src_width,
                                              int          src_height,
                                              float*       dst,
                                              ...
                                              cudaStream_t stream)
{
    ...
    checkCudaKernel(warp_affine_bilinear_and_normalize_focus_kernel<<<grid, block, 0, stream>>>(
        src, src_line_size, src_width, src_height, dst, dst_width, dst_height, const_value, matrix_1_3, norm, jobs));
}

void warp_perspective(uint8_t*     src,
                      int          src_line_size,
                      int          src_width,
                      int          src_height,
                      float*       dst,
                      ...
                      cudaStream_t stream)
{
    ...
    checkCudaKernel(warp_perspective_kernel<<<grid, block, 0, stream>>>(
        src, src_line_size, src_width, src_height, dst, dst_width, dst_height, const_value, matrix_3_3, norm, jobs));
}

void resize_bilinear_and_normalize(
    uint8_t* src, int src_line_size, int src_width, int src_height, float* dst, int dst_width, int dst_height, const Norm& norm, cudaStream_t stream)
{
    ...
    checkCudaKernel(resize_bilinear_and_normalize_kernel<<<grid, block, 0, stream>>>(src,
                                                                                     src_line_size,
                                                                                     src_width,
                                                                                     src_height,
                                                                                     dst,
                                                                                     dst_width,
                                                                                     dst_height,
                                                                                     src_width / (float)dst_width,
                                                                                     src_height / (float)dst_height,
                                                                                     norm,
                                                                                     jobs));
}

void norm_feature(float* feature_array, int num_feature, int feature_length, cudaStream_t stream)
{
    ...
    checkCudaKernel(normalize_feature_kernel<<<grid, block, num_feature * sizeof(float), stream>>>(feature_array, num_feature, feature_length, jobs));
}


__global__ void crop_resize_bilinear_and_normalize_kernel(uint8_t* src,
                                                          int      src_line_size,
                                                          int      src_width,
                                                          int      src_height,
                                                          float*   dst,
    ...
    uint8_t* src, int src_line_size, int src_width, int src_height, float* dst, int dst_width, int dst_height, const Norm& norm, cudaStream_t stream)
{
    ...
    checkCudaKernel(crop_resize_bilinear_and_normalize_kernel<<<grid, block, 0, stream>>>(
        src, src_line_size, src_width, src_height, dst, dst_width, dst_height, crop_x, crop_y, sx, sy, norm, jobs));
}


__global__ void resize_normalize_image_kernel(uint8_t* src,
                                              int      src_line_size,
                                              int      src_width,
                                              int      src_height,
                                              float*   dst,
...
void resize_normalize_image(uint8_t*     src,
                            int          src_line_size,
                            int          src_width,
                            int          src_height,
                            float*       dst,
                            ...
                            cudaStream_t stream)
{
    ...
    checkCudaKernel(resize_normalize_image_kernel<<<grid, block, 0, stream>>>(src,
                                                                              src_line_size,
                                                                              src_width,
                                                                              src_height,
                                                                              dst,
