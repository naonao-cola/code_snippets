...
public:
    ...
    virtual void worker(promise<bool>& result) override
    {
        ...
        // 新增
        int output_size_three = output->size(2);

        const int MAX_IMAGE_BBOX  = max_objects_;
        const int NUM_BOX_ELEMENT = 4 + 1 + 1 + 1 + 3 * key_points_;   // left, top, right, bottom, cls, confidence, keepflag, (x, y, conf) * 17
        ...
        while (get_jobs_and_wait(fetch_jobs, max_batch_size)) {
            ...
            for (int ibatch = 0; ibatch < infer_batch_size; ++ibatch) {
                ...
                for (int i = 0; i < count; ++i) {
                    ...
                    int    keepflag = pbox[6];
                    if (keepflag == 1) {
                        Box    box(pbox[0], pbox[1], pbox[2], pbox[3], pbox[4], pbox[5], key_points_);
                        float* pkeypoint_start = pbox + 7;
                        float* pkeypoint_end   = pkeypoint_start + 3 * key_points_;
                               ...
                               bool          use_multi_preprocess_stream)
{
    ...
    if (!instance->startup(
            engine_file, gpuid, confidence_threshold, nms_threshold, nms_method, max_objects, key_points, cls_num, use_multi_preprocess_stream)) {
        instance.reset();
    }
    return instance;
}
