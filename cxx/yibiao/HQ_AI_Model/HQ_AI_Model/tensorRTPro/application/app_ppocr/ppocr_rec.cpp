/**
 * @FilePath     : /HQ_AI_Model/HQ_AI_Model/HQ_AI_Model/tensorRTPro/application/app_ppocr/ppocr_rec.cpp
 * @Description  :
 * @Author       : weiwei.wang
 * @Date         : 2025-11-27 08:51:04
 * @Version      : 0.0.1
 * @LastEditors  : weiwei.wang
 * @LastEditTime : 2025-11-27 10:20:58
...
public:
    ...
    virtual bool startup(const string& engine_file, int gpuid, const string& dict_path, int rec_batch_num, bool use_multi_preprocess_stream)
    {
...
shared_ptr<TextRecognizer> create_recognizer(
    const string& engine_file, int gpuid, const string& dict_path, int rec_batch_num, bool use_multi_preprocess_stream)
{
    shared_ptr<TextRecognizerImpl> instance(new TextRecognizerImpl());
    if (!instance->startup(engine_file, gpuid, dict_path, rec_batch_num, use_multi_preprocess_stream)) {
        instance.reset();
    }
    return instance;
}
