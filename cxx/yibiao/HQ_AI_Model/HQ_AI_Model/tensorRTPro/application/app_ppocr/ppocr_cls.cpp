...
public:
    ...
    virtual bool startup(const string& engine_file, int gpuid, float cls_thresh, int cls_batch_num, bool use_multi_preprocess_stream)
    {
    ...
    virtual void setConfidence(float confidence_threshold)
    {
        ...
        cls_thresh_ = confidence_threshold;
        return;
    }
...
shared_ptr<TextClassifier> create_classifier(
    const string& engine_file, int gpuid, float cls_thresh, int cls_batch_num, bool use_multi_preprocess_stream)
{
    shared_ptr<TextClassifierImpl> instance(new TextClassifierImpl());
    if (!instance->startup(engine_file, gpuid, cls_thresh, cls_batch_num, use_multi_preprocess_stream)) {
        instance.reset();
    }
    return instance;
}
