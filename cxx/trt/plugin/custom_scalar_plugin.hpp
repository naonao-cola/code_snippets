...
public:
    ...
    */
    CustomGridSamplePlugin() = delete; //默认构造函数，一般直接delete
    CustomGridSamplePlugin(const std::string &name);  //parse, clone时候用的构造函数
    CustomGridSamplePlugin(const std::string &name,const void* buffer, size_t length); //反序列化的时候用的构造函数
...
public:
CustomGridSamplePluginCreator();  //初始化mFC以及mAttrs
