...
int NmsCl::Init(const std::string& cl_path)
{
    ...
    if (commandQueue == NULL) {
        ...
        std::cout << "Failed to create OpenCL commandQueue.";
        return -1;
    }
    ...
    if (program == NULL) {
        Cleanup(context, commandQueue, program, kernel, memObjects);
        std::cout << "Failed to create OpenCL program." << cl_path;
        return -1;
    }
    ...
    if (kernel == NULL) {
        std::cout << "Failed to create kernel" << cl_kernel_name;
        Cleanup(context, commandQueue, program, kernel, memObjects);
        return -1;
    }
    std::cout << "Succeed to init cl " << cl_path << " with kernel " << cl_kernel_name<<std::endl;
    return 0;
}
...
int NmsCl::Forward(float* box, const int& box_nums, const float& iou_thr, std::vector<int>& keep_box)
{
    ...
    if (!CreateMemObjects(context, memObjects, box, box_num, global_work_size[0], global_work_size[1])) {
        Cleanup(context, commandQueue, program, kernel, memObjects);
        std::cout << "Error kernel CreateMemObjects.";
        return -1;
    }
    cl_int errNum;
    ...
    if (errNum != CL_SUCCESS) {
        std::cout << "Error setting kernel arguments.";
        Cleanup(context, commandQueue, program, kernel, memObjects);
        return -1;
    }
    ...
    errNum = clEnqueueNDRangeKernel(commandQueue, kernel, 2, NULL,
        ...
        0, NULL, NULL);
    if (errNum != CL_SUCCESS) {
        std::cout << "Error queuing kernel for execution.";
    ...
    if (errNum != CL_SUCCESS) {
        std::cout << "Error reading result buffer. " << errNum << std::endl;
        Cleanup(context, commandQueue, program, kernel, memObjects);
        return 1;
    }
